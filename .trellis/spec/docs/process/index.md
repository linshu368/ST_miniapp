...
- 如果改动涉及根 `AGENTS.md`、package spec 或 agent 流程，先确认规则来源与同步责任；已删除的 package `CLAUDE.md` 不得恢复为平行来源。
...
- 当根 `AGENTS.md`、Backend/Frontend package spec 或 agent 工作流更新时，必须同步评估以下 Trellis 文档是否需要更新：
...
- 对需要长期维护的实现与规则变更，要同时考虑必要注释、相关文档与测试必要性，并在检查环节显式核对；默认不因代码变化自动创建测试文件，复杂逻辑只补解释“为什么”的必要简洁注释。
...
- 检查根 `AGENTS.md`、package spec 与 Trellis 承接文档之间是否存在漂移或矛盾。
