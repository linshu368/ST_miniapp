...
- `api/telemetry.ts` 变更必须覆盖禁止键、`user_cohort` 不得出现、`payment_return_observed.return_source` 的 `webview_resume`，以及 `recharge_entry_clicked` 允许没有 `replay_context_id`；frontend/backend 事件发送方不得私自加字段。
