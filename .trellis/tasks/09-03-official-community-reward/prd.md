...
- 入口显隐由后端运行时总开关控制。总开关关闭时入口隐藏且停止新发奖，不修改历史领奖、钱包流水或通知。
...
1. **已确认：**不复用 MiniApp 登录 Bot；使用 env 指定的独立 Community Bot 作为官方群管理员，接收 `chat_member` 并执行 `getChatMember`。
2. **已确认：测试与生产均使用 `@MijingAI_Official`，公开链接为 `https://t.me/MijingAI_Official`。** 仍需获取该群的稳定数字 `chat_id`（通常是 `-100...`），后端用它过滤 `chat_member/getChatMember`；不能用 `@MijingAI_Official` 替代数据库幂等键。
