...
packages/preset-platform (Vite React)
  └─ HTTPS/SSE + Supabase access token
      → packages/backend /api/internal/preset-platform/*
          ├─ Admin Supabase 账号/角色鉴权、Preset Platform 授权、会话/预设编排
...
浏览器不直连 OpenRouter、不读取原始 `experience` 消息、不持有 service role。Supabase browser client 仅负责复用 Admin 账号体系的登录/session；Preset Platform 授权关系和业务数据独立落在 `preset_platform`，业务数据统一经 backend，确保脱敏和权限处于可信边界。
...
本需求有四个核心工作区、大量服务器状态及环境切换失效需求，因此使用 Router + Query，而不复制 Admin 的单文件 view 状态机。

```text
src/
├─ app/             # providers、router、guards、error boundary
├─ layouts/         # WorkspaceLayout、环境标识、侧栏
├─ pages/           # conversations/models/presets/materials
...
状态归属：URL 保存页面/筛选/选中 ID；React Query 保存服务器真相；表单/抽屉/输入框用局部状态；首期不引入第二个全局状态源。会话和草稿均按 operator 私有过滤，已发布预设与平台默认预设可被同环境有权限成员查看/应用。

页面信息架构：

| 页面            | 主要内容                                                                 | 交互重点                                                                                                                                                      |
| --------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 对话测试        | 左侧私有会话列表、中间对话流、右侧会话配置/当前环境角色卡/脱敏输入快捷区 | 参照 `0904-预设对话平台-交互预览.html` 图 1；模型为 OpenRouter 下拉选择；点击预设打开右侧抽屉                                                                 |
| OpenRouter 模型 | 模型目录、搜索、筛选、缓存状态、刷新                                     | 参考 Admin `ModelCatalogEditor` / `openRouterModels` 的目录获取、shared schema 校验、搜索和强刷处理；正常可调用模型均可用于当前会话；刷新受运营权限和限频控制 |
| 预设管理        | `平台预设` / `预设库` / `草稿箱` 三个 Tab                                | 平台默认发布/回滚、已发布预设查看/应用、私有草稿编辑/应用在同一模块完成                                                                                       |
| 测试素材        | 当前环境角色卡选择、脱敏真实输入列表                                     | 角色卡按环境只读导入；真实输入仅展示脱敏文本和 opaque sample                                                                                                  |

对话测试页的预设抽屉只负责快速选择：顶部展示当前平台默认状态，列表合并“已发布预设”和“我的草稿”，提供应用、查看、编辑入口；发布和回滚仍在预设管理页完成。
...
新建独立 `preset_platform` schema，首期全部工作台资产、数据访问函数和授权策略均归该域管理，**不归属 `admin` schema**。测试环境和生产环境均沿用 Admin Supabase 账号/角色表作为认证来源，但不把 Preset Platform 的表、RPC、RLS、grant、审计或生命周期并入 Admin。生产模式允许在生产库独立 `preset_platform` 域保存工作台数据；不得写入 `admin` 或 MiniApp 业务域。若未来 MiniApp 正式消费，再用 migration 将已发布运行时快照投影到 `app_core`，现在不双写。
...
| `preset_platform.test_sessions`      | operator 私有、标题、model、default/pinned binding、角色快照、busy/revision。 |
| `preset_platform.test_messages`      | turn/role/content/status/request_id、模型/预设/角色快照、finish/error。       |
| `preset_platform.audit_log`          | 发布、回滚、覆盖、测试素材读取的安全摘要，不记正文/个人数据。                 |

数据库不变量：migration 文件头声明 `-- domain: preset_platform`；UUID/timestamptz、显式 check/FK/index/comment；release immutable；默认 pointer 只指有效 release；原子 RPC 锁 session 分配 turn；request ID unique；会话和草稿按 operator 私有约束；RLS/grant 按 viewer/operator/publisher；`SECURITY DEFINER` 固定 search_path、撤销 PUBLIC execute。Preset Platform repository 只能以按域 client/RPC 访问自有域，跨域角色卡与真实输入仍遵循只读 repository 边界，不直接 JOIN。
...
模型目录前端处理参考 Admin：`packages/admin/src/lib/openRouterModels.ts` 的 `fetchOpenRouterModels(environment, forceRefresh)` 负责按环境 API base 拉取 `/api/platform/openrouter/models` 并用 `OpenRouterModelDirectorySchema` 校验；`packages/admin/src/components/ModelCatalogEditor.tsx` 的 `filterOpenRouterModels()` 支持按模型名称、OpenRouter ID、描述和 `canonical_slug` 搜索，并展示 stale/同步状态、上游更新时间、模型总数、过期状态和强制刷新入口。Preset Platform 不复用 Admin 组件代码，但应复用这套交互和校验口径，在专用内部端点上补 operator 鉴权与 refresh 限频。
...
4. 固定组装：单段 preset system instruction + character system prompt + 成功历史 + 当前输入；用 golden test 固化。
...
服务端按当前环境、角色 + 时间 cursor 有界查询当前有效 user turn，仅取必要列。依次执行权限校验、丢弃空白/过短/系统内容、用户 ID/Telegram 标识/手机号/邮箱/URL query/身份与支付标识脱敏、生成 opaque 临时引用。绝不返回 user/session/message ID。首期不做内容审计或语义风险审核；规则脱敏只能降低风险，必须同时使用最小窗口、权限和禁止日志原文。
...
默认 test，目标配置不完整则不可选，禁止回退 production。环境切换沿用 Admin 的成组配置思路：API URL、Supabase URL/anon key、session、React Query cache、SSE request 和环境指纹必须整体切换。Backend 扩展 CORS 加 `PRESET_PLATFORM_URL`；每个 Railway environment 仅持自身 `DATABASE_ENV/SUPABASE_*` 与现有 `LLM_*`，不让一个进程同时持两套 service role。
...
新建独立 Vercel Project。部署配置参考现有 Admin 与 CS Platform：

| 项              | Admin / CS 现状                                                                                                                                                                                          | Preset Platform 规划                                                                                                                                                                                                                             |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Vercel 配置     | Admin 包内 `vercel.json` 使用 `pnpm --filter @miniapp/admin build`、`outputDirectory=dist`、SPA rewrite；CS 使用 `pnpm --filter @miniapp/cs-platform build`、`outputDirectory=packages/cs-platform/dist` | 新包提供 `packages/preset-platform/vercel.json`，`framework=null`、`installCommand=pnpm install`、`buildCommand=pnpm --filter @miniapp/preset-platform build`、`outputDirectory` 按 Vercel Root Directory 决定；需要 SPA rewrite 到 `index.html` |
| Vercel 环境变量 | 浏览器公开变量使用 `VITE_*` / `NEXT_PUBLIC_*`，secret 不进 Vercel 前端包                                                                                                                                 | 配置 `VITE_PRESET_TEST_*` 和 `VITE_PRESET_PROD_*`；Preview 默认只填 test，production 才填 prod；缺任一目标环境变量则禁用对应环境                                                                                                                 |
| Railway         | `.railway/railway.ts` 只管理 `stminiapp` backend 与支付任务，前端不在 Railway                                                                                                                            | 不新增 Railway service；仅在 backend 变量清单和实际环境补 `PRESET_PLATFORM_URL`，必要时给 preview/prod 分别配置 allow-origin                                                                                                                     |
| Preview         | `railway-pr-env.yml` 为 PR 创建 `pr-{N}` backend，Vercel Preview 直连该 backend URL                                                                                                                      | Preset Platform Vercel Preview 采用同样模式：默认 test Supabase，API URL 指向 PR backend 或 development backend；不得连接 production                                                                                                             |

CI 参考现有 `.github/workflows/ci.yml`：根 `pnpm typecheck` 和 `pnpm lint` 会覆盖 workspace；需要显式增加 `pnpm --filter @miniapp/preset-platform test` 与 `pnpm --filter @miniapp/preset-platform build`。Docker/GHCR workflow 不增加 Preset Platform 镜像；生产仍由 Vercel 构建静态产物。Railway PR environment workflow 仍只复制 backend service，后续若 Preset Platform Preview 需要自动评论 URL，可在 Vercel 集成侧处理，不把前端部署搬到 Railway。

Preview 默认只能连 test；production 功能 flag 默认关闭，安全验收后按角色开放。
...
| 脱敏漏检          | 最小字段/时间窗、关键用户信息规则脱敏、权限、禁日志               | 不做内容审计或语义审核 |
| DB 部分成功       | 同库不变量 RPC transaction；外部调用明确提交点/失败收口           | 不做分布式事务         |
| 容量              | 分页、长度/窗口/并发/时间上限、保留策略                           | 首期不建归档服务       |

建议初值（待确认）：单操作者 2 并发、同会话 1；20 次/分钟；输入 8 KiB、预设/回复 32 KiB；整体 120s；最近 50 轮或 token 预算先到。正常可调用的 OpenRouter 模型均可选择，首期不设高价模型 allowlist。

## 7. 安全、隐私与可观测性

- 浏览器不可信；权限、校验、只读约束落在 backend/repository/RLS/grant。
- 生产读取 repository 不提供 update/delete；数据库角色尽量只授 SELECT/RPC。
- 角色卡/输入均视为不可信文本，不解释为工具命令；首期禁 tool calling；不做内容审计。
- Pino allowlist：环境、opaque operator、session/request/model/release、状态、耗时、计数；禁止正文、token、key、原始 sample ID。
- 审计发布、回滚和测试素材访问。

事件建议：`model_catalog.fetch`、`generation` 各阶段、`publish`、`material.read`、`environment_mismatch`、`rate_limited`、`session_busy`。出现未脱敏关键用户信息、跨环境、未授权生产读取或业务表写入时，立即关闭 production flag 并 revoke 权限。
...
SPA/backend 可回滚；数据库 expand-first，旧版本忽略新表。错误默认预设通过 pointer 回滚上一 release，不删历史。已有数据后 migration 优先 forward-fix。测试素材风险用 flag + revoke 立即止血。内部生成路由/限流隔离，不得耗尽 MiniApp 正式流量池。

## 9. 难点

1. 在 generation 唯一出口内复用 provider，又不触发 MiniApp 钱包/历史副作用。
2. V1 既满足质量验证又不重建 ST；建议固定顺序、单 system instruction。
3. 关键用户信息脱敏无法靠正则保证完全覆盖，需最小化、权限、禁日志和人工 fixture 验证；首期不做内容审计。
