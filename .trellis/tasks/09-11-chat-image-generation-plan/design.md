...
- 输入快照：`prompt_cn`（用户确认/输入的中文短文）、`prompt_source=generated|custom`、必要时保存内部 `prompt_en` / `provider_prompt` 审计字段（不公开）。
...
- `POST /api/v1/conversations/:sessionId/messages/:messageId/image-description`：仅默认路径使用，生成免费中文描述；请求不接受模型名。
- `POST /api/v1/conversations/:sessionId/messages/:messageId/images`：body `{ prompt_cn, prompt_source }`，余额预检和 200 字校验后原子创建 pending，返回 202；`prompt_source=custom` 不触发分镜写稿，但后续仍触发中文到英文直译。

不增加“确认旧 description id”的服务端状态：描述预览是短生命周期 UI 数据，确认时直接提交最终 `prompt_cn` 可减少表、过期策略和一次读取。后端仍重新做 ownership、最后完整回复、长度和安全校验。
...
- DeepSeek 复用：写稿与翻译若使用语音同款 `deepseek-v4-flash`，复用现有 `DEEPSEEK_API_KEY`、`DEEPSEEK_URL`、`DEEPSEEK_MODEL` 配置和 OpenAI-compatible transport；建议抽出 `features/generation/deepseek-text.ts`，保留 `config.voice.draft` 兼容或迁移到通用 `config.deepseek`。
