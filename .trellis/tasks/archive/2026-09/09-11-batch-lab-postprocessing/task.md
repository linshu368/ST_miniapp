...
- 2026-09-16：已知非本任务阻塞：`pnpm lint:migrations` 仍因已执行旧文件 `20260920_batch_lab_samples.sql` 的三位数字命名失败；本任务新增 migration 使用 `YYYYMMDD_...`。`pnpm lint:imports` 仍因现有 ESLint 命令 `--rule '{}'` 解析失败。
