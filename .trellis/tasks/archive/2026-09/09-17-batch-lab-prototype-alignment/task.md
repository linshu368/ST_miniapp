...
- 2026-09-18: Fixed sample preview SQL placeholder mismatch: backend source SQL compilation now accepts both `:name` and `{{name}}` placeholders, and the Batch Lab default sample SQL now shows `:min_turn`. Verification passed for backend Batch Lab tests, backend typecheck, Batch Lab tests, typecheck, and build.
