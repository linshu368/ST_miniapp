...
Every task has its own directory under `.trellis/tasks/{MM-DD-name}/` holding `task.json`, `prd.md`, `task.md`, optional `design.md`, optional `implement.md`, optional `research/`, and context manifests (`implement.jsonl`, `check.jsonl`) for sub-agent-capable platforms.
...
- 1.1 Requirement exploration `[required · repeatable]` (`prd.md`; complex tasks also need `design.md` + `implement.md` + `task.md`)
...
5. Artifact presence informs the next step; missing `design.md` / `implement.md` / `task.md` is valid for lightweight tasks and incomplete planning for complex tasks.
...
- PRD-only is valid for lightweight tasks; complex ST_miniapp tasks need `design.md` + `implement.md` + `task.md`.
...
- For complex tasks, produce `design.md`, `implement.md`, and `task.md` before implementation starts
...
| `task.md` exists and is broken down into executable tasks (complex tasks) |     ✅      |
...
- Planning must distinguish lightweight PRD-only tasks from complex tasks that require `prd.md`, `design.md`, `implement.md`, and `task.md` before start.
