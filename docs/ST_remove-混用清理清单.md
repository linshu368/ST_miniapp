...
| `ops/env/vercel.env.production.example`        | **`ST_PUBLIC_PROXY_URL` 不能删**：它仍被 `frontend/src/app/api/lobby-characters/route.ts` 当作 backend 地址兜底。但模板里「next.config.mjs 据此启用 rewrites 转发 /tavern/\*」已是错的（rewrites 在第一批就删了），改成如实描述；改名要连 Vercel 变量一起动，归网关收敛那一档 |
