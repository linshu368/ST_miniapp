...
import {
  ...
  saveSettings as saveExtensionSettings,
...
function initChatDisplaySwitcher() {
  ...
  function applyChatDisplayStyle() {
    ...
    switch (chatDisplaySelect.value) {
      ...
      case '6':
        ...
        break;
      case '7':
        document.body.classList.add('tidestyle');
        break;
    }
  ...
  // 平台 provision 的 settings.json 是主题真相源；首次启动没有 localStorage 时也要恢复 Echo。
  if (
    power_user &&
    typeof power_user.chat_display === 'number' &&
    power_user.chat_display >= 3 &&
    power_user.chat_display <= 7
  ) {
    chatDisplaySelect.value = String(power_user.chat_display);
  }
...
export function applyThemeSetting(varId, value) {
  ...
  document.dispatchEvent(
    new CustomEvent('themeSettingChanged', {
      detail: { varId, value },
    })
  );
}
...
export function addCustomSetting(settingConfig) {
  ...
  if (settingsContainer) {
    ...
    const enabledCheckbox = settingsContainer.querySelector(
      `#${settingsKey}-enabled`
    ).parentElement;
