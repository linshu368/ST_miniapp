...
class PromptManager {
  ...
  init(moduleConfiguration, serviceSettings) {
    ...
    this.handleResetPrompt = (event) => {
      ...
      const nameField = /** @type {HTMLInputElement} */ (
        document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_name')
      );
      const roleField = /** @type {HTMLSelectElement} */ (
        document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_role')
      );
      ...
      const forbidOverridesBlock = /** @type {HTMLDivElement} */ (
        document.getElementById(this.configuration.prefix + 'prompt_manager_forbid_overrides_block')
      );
    ...
    this.handleAppendPrompt = (event) => {
      const appendPromptFooter = /** @type {HTMLSelectElement} */ (
        document.getElementById(this.configuration.prefix + 'prompt_manager_footer_append_prompt')
      );
  ...
  checkForMissingPrompts(prompts) {
    const defaultPromptIdentifiers = chatCompletionDefaultPrompts.prompts.reduce((list, prompt) => {
      list.push(prompt.identifier);
      return list;
    }, []);

    const missingIdentifiers = defaultPromptIdentifiers.filter(
      (identifier) => !prompts.some((prompt) => prompt.identifier === identifier)
    );
  ...
  getPromptOrderEntry(character, identifier) {
    return (
      this.getPromptOrderForCharacter(character).find((entry) => entry.identifier === identifier) ??
      null
    );
  }
  ...
  getPromptById(identifier) {
    return (
      this.serviceSettings.prompts.find((item) => item && item.identifier === identifier) ?? null
    );
  }
  ...
  loadPromptIntoEditForm(prompt) {
    ...
    const entrySourceBlock = /** @type {HTMLDivElement} */ (
      document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_source_block')
    );
    const entrySource = /** @type {HTMLSpanElement} */ (
      document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_source')
    );
    ...
    Array.from(injectionTriggerField.options).forEach((option) => {
      option.selected =
        Array.isArray(prompt.injection_trigger) && prompt.injection_trigger.includes(option.value);
    });
    injectionTriggerField.dispatchEvent(new Event('change', { bubbles: true }));
  ...
  loadMessagesIntoInspectForm(messages) {
    ...
    const createInlineDrawer = (message) => {
      const truncatedTitle =
        message.content.length > 32 ? message.content.slice(0, 32) + '...' : message.content;
  ...
  async renderPromptManager() {
    ...
    if (null !== this.activeCharacter) {
      ...
      const promptsHtml = prompts.reduce(
        (acc, prompt) =>
          acc + `<option value="${prompt.identifier}">${escapeHtml(prompt.name)}</option>`,
        ''
      );
  ...
  async renderPromptManagerListItems() {
    ...
    this.getPromptsForCharacter(this.activeCharacter).forEach((prompt) => {
      ...
      const iconLookup =
        prompt.role === 'system' && (prompt.marker || prompt.system_prompt) ? '' : prompt.role;
    ...
    Array.from(promptManagerList.getElementsByClassName('prompt-manager-detach-action')).forEach(
      (el) => {
        el.addEventListener('click', this.handleDetach);
      }
    );

    Array.from(promptManagerList.getElementsByClassName('prompt-manager-inspect-action')).forEach(
      (el) => {
  ...
  makeDraggable() {
    $(`#${this.configuration.prefix}prompt_manager_list`).sortable({
      ...
      update: (event, ui) => {
        const promptOrder = this.getPromptOrderForCharacter(this.activeCharacter);
        const promptListElement = $(`#${this.configuration.prefix}prompt_manager_list`).sortable(
          'toArray',
