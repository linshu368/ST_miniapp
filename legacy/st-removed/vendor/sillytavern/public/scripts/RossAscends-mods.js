...
export async function RA_CountCharTokens() {
  ...
  const tokenLimit = Math.max(
    (main_api !== 'openai' ? max_context : oai_settings.openai_max_context) / 2,
    1024
  );
      ...
      case 'openai':
        if (
          ...
          (secret_state[SECRET_KEYS.AI21] &&
            oai_settings.chat_completion_source == chat_completion_sources.AI21) ||
          ...
          (secret_state[SECRET_KEYS.COHERE] &&
            oai_settings.chat_completion_source == chat_completion_sources.COHERE) ||
          ...
          (secret_state[SECRET_KEYS.GROQ] &&
            oai_settings.chat_completion_source == chat_completion_sources.GROQ) ||
          (secret_state[SECRET_KEYS.CHUTES] &&
          ...
          (secret_state[SECRET_KEYS.XAI] &&
            oai_settings.chat_completion_source == chat_completion_sources.XAI) ||
          ...
          (secret_state[SECRET_KEYS.ZAI] &&
            oai_settings.chat_completion_source == chat_completion_sources.ZAI) ||
...
export function initRossMods() {
  ...
  if (!isMobile()) {
    //only read/set pin states on non-mobile devices
  ...
  $(SelectedCharacterTab).on('click', function () {
  ...
  async function processHotkeys(event) {
            ...
            'Are you sure you want to regenerate the latest message?',
            {
              ...
              onClose: (popup) => {
                regenerateWithCtrlEnter = Boolean(
                  popup.inputResults.get('regenerateWithCtrlEnter') ?? false
                );
              },
            }
          );
    ...
    if (event.key == 'Escape') {
      ...
      if ($('#left-nav-panel').is(':visible') && $(LPanelPin).prop('checked') === false) {
        $('#leftNavDrawerIcon').trigger('click');
        return;
      }

      if ($('#right-nav-panel').is(':visible') && $(RPanelPin).prop('checked') === false) {
        $('#rightNavDrawerIcon').trigger('click');
        return;
      }
