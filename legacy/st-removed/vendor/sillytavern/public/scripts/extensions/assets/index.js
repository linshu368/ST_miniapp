...
function createAssetButton(asset, assetType, index) {
  ...
  const assetDelete = async function () {
    if (assetType === 'character') {
      toastr.error(
        'Go to the characters menu to delete a character.',
...
function createAssetBlock(asset, assetType, element) {
  ...
  const title =
    assetType === 'extension' ? t`Extension repo/guide:` + ` ${url}` : t`Preview in browser`;
  const previewIcon =
    assetType === 'extension' || assetType === 'character'
  ...
  const nameSpan = $('<span>', { class: 'asset-name flex-container alignitemscenter' })
    ...
    .append(
      $('<a>', { class: 'asset_preview', href: url, target: '_blank', title: title }).append(
        $('<i>', { class: `fa-solid fa-sm ${previewIcon}` })
      )
    );
  ...
  const infoDiv = $('<div>', {
    class: 'flex-container flexFlowColumn flexNoGap wide100p overflowHidden',
  })
    .append(nameSpan)
  ...
  if (assetType === 'character') {
    ...
    nameSpan.prepend(
      $('<div>', { class: 'avatar' }).append($('<img>', { src: asset.url, alt: displayName }))
    );
  }
...
async function buildAssetTypeSection(assetType) {
  ...
  for (const asset of availableAssets[assetType].sort(
...
async function populateAssetsMenu(json) {
  ...
  const assetTypes = Object.keys(availableAssets).sort((a, b) =>
    a === 'extension' ? -1 : b === 'extension' ? 1 : 0
  );
...
async function downloadAssetsList(url) {
  ...
  try {
    ...
    toastr.info(
      'Click the flashing button at the top right corner of the menu.',
      'Trying to install a custom extension?',
      { timeOut: 10_000 }
    );
...
function isAssetInstalled(assetType, filename) {
  ...
  for (const i of assetList) {
    ...
    if (i.includes(filename)) return true;
  }
...
async function openCharacterBrowser(forceDefault) {
  ...
  for (const character of characters.sort((a, b) => a.name.localeCompare(b.name))) {
    const listElement = template.find(
      character.highlight ? '.contestWinnersList' : '.featuredCharactersList'
    );
    const characterElement = $(
      await renderExtensionTemplateAsync(MODULE_NAME, 'character', character)
    );
  ...
  connectButton.on('click', async function () {
    ...
    const confirmation =
      ...
      (await Popup.show.confirm(
        t`Loading Asset List`,
        '<span>' +
          t`Are you sure you want to connect to the following url?` +
