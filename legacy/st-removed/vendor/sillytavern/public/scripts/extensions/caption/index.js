...
async function captionExistingMessage(message, mediaIndex) {
  ...
  if (
    ...
    isNaN(mediaIndex) ||
    mediaIndex < 0 ||
    mediaIndex >= message.extra.media.length
  ) {
    mediaIndex = 0;
  }
...
async function captionMultimodal(base64Img, externalPrompt) {
  ...
  if (!externalPrompt && extension_settings.caption.prompt_ask) {
    const customPrompt = await callGenericPopup(
    ...
    if (Array.isArray(message?.extra?.media) && message.extra.media.length > 0) {
      try {
        ...
        if (mediaAttachment.type === MEDIA_TYPE.AUDIO) {
          toastr.error(
            'The specified media is an audio file. Captioning audio files is not supported.'
          );
          return '';
        }
        if (mediaAttachment.type === MEDIA_TYPE.VIDEO && !isVideoCaptioningAvailable()) {
          toastr.error(
            'The specified media is a video. Captioning videos is not supported for the current source.'
          );
          return '';
        }
...
export async function init() {
  function addSendPictureButton() {
    ...
    $(sendButton).on('click', () => {
      ...
      if (!hasCaptionModule) {
        toastr.error(
          'Choose other captioning source in the extension settings.',
          'Captioning is not available'
        );
        return;
      }
  ...
  async function switchMultimodalBlocks() {
    ...
    if (!extension_settings.caption.multimodal_model) {
      ...
      const options = dropdown.find(
        `option[data-type="${extension_settings.caption.multimodal_api}"]`
      );
  ...
  async function addRemoteEndpointModels() {
      ...
      if (Array.isArray(modelIds) && modelIds.length > 0) {
        modelIds.sort().forEach((modelId) => {
          if (
            ...
            options.some((o) => o.value === modelId && o.dataset.type === api)
          ) {
            return;
          }
    ...
    await processEndpoint(
      'pollinations',
      '/api/backends/chat-completions/multimodal-models/pollinations'
    );
    ...
    await processEndpoint(
      'electronhub',
      '/api/backends/chat-completions/multimodal-models/electronhub'
    );
    ...
    await processEndpoint(
      'workers_ai',
      '/api/backends/chat-completions/multimodal-models/workers_ai',
  ...
  $('#caption_allow_reverse_proxy').prop(
    'checked',
  ...
  $('#caption_allow_reverse_proxy').on('input', () => {
    extension_settings.caption.allow_reverse_proxy = $('#caption_allow_reverse_proxy').prop(
      'checked'
    );
    saveSettingsDebounced();
  });
  ...
  $('#caption_altEndpoint_url')
    .val(extension_settings.caption.alt_endpoint_url)
    .on('input', () => {
      extension_settings.caption.alt_endpoint_url = String($('#caption_altEndpoint_url').val());
      saveSettingsDebounced();
    });
  $('#caption_altEndpoint_enabled')
    .prop('checked', !!extension_settings.caption.alt_endpoint_enabled)
    .on('input', () => {
      extension_settings.caption.alt_endpoint_enabled = !!$('#caption_altEndpoint_enabled').prop(
        'checked'
      );
      saveSettingsDebounced();
    });
  $('#caption_show_in_chat')
    .prop('checked', !!extension_settings.caption.show_in_chat)
    .on('input', () => {
      extension_settings.caption.show_in_chat = !!$('#caption_show_in_chat').prop('checked');
      saveSettingsDebounced();
    });
  $('#caption_ollama_custom_model')
    .val(extension_settings.caption.ollama_custom_model || '')
    .on('input', () => {
      extension_settings.caption.ollama_custom_model = String(
        $('#caption_ollama_custom_model').val()
      ).trim();
      saveSettingsDebounced();
    });
  $('#caption_custom_model')
    .val(extension_settings.caption.custom_model || '')
    .on('input', () => {
    ...
    if (Array.isArray(message?.extra?.media) && message.extra.media.length > 0) {
      for (let mediaIndex = 0; mediaIndex < message.extra.media.length; mediaIndex++) {
        ...
        try {
          ...
          console.error(
            `Auto-captioning failed for message ID ${messageId}, media index ${mediaIndex}`,
            e
          );
          continue;
        }
    ...
    SlashCommand.fromProps({
      ...
      namedArgumentList: [
        new SlashCommandNamedArgument(
          'quiet',
          'suppress sending a captioned message',
