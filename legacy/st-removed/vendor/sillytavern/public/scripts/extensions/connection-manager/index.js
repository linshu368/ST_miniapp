...
async function createConnectionProfile(forceName = null) {
  ...
  const template = $(
    await renderExtensionTemplateAsync(MODULE_NAME, 'profile', { profile: profileForDisplay })
  );
...
function renderConnectionProfiles(profiles) {
  ...
  for (const profile of extension_settings.connectionManager.profiles.sort((a, b) =>
...
async function generateStreamCallback(args, value) {
  ...
  const onCompleteClosure =
    args?.onComplete instanceof SlashCommandClosure ? args.onComplete : null;
...
export async function init() {
  extension_settings.connectionManager =
    extension_settings.connectionManager || structuredClone(DEFAULT_SETTINGS);
  ...
  function toggleProfileSpecificButtons() {
    ...
    profileSpecificButtons.forEach((id) =>
      document.getElementById(id).classList.toggle('disabled', !profileId)
    );
  }
  ...
  editButton.addEventListener('click', async () => {
    ...
    const sortByViewOrder = (a, b) =>
      Object.keys(FANCY_NAMES).indexOf(a) - Object.keys(FANCY_NAMES).indexOf(b);
    ...
    const template = $(
      await renderExtensionTemplateAsync(MODULE_NAME, 'edit', { name: profile.name, settings })
    );
    ...
    if (
      ...
      extension_settings.connectionManager.profiles.some((p) => p.name === newName)
    ) {
      toastr.error('A profile with the same name already exists.');
      return;
    }
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      callback: () =>
        JSON.stringify(extension_settings.connectionManager.profiles.map((p) => p.name)),
    })
  );
