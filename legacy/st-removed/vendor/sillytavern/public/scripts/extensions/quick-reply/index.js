      ...
      extension_settings.quickReplyV2 = {
        ...
        config: {
          setList: [
            {
              set:
                extension_settings.quickReply.selectedPreset ??
                extension_settings.quickReply.name ??
                'Default',
              isVisible: true,
            },
          ],
...
eventSource.makeFirst(event_types.USER_MESSAGE_RENDERED, (...args) =>
  executeIfReadyElseQueue(onUserMessage, args)
);
...
eventSource.makeFirst(event_types.CHARACTER_MESSAGE_RENDERED, (...args) =>
  executeIfReadyElseQueue(onAiMessage, args)
);

const onGroupMemberDraft = async () => {
  await autoExec.handleGroupMemberDraft();
};
eventSource.on(event_types.GROUP_MEMBER_DRAFTED, (...args) =>
  executeIfReadyElseQueue(onGroupMemberDraft, args)
);
...
eventSource.on(event_types.WORLD_INFO_ACTIVATED, (...args) =>
  executeIfReadyElseQueue(onWIActivation, args)
);
...
eventSource.on(event_types.GROUP_CHAT_CREATED, (...args) =>
  executeIfReadyElseQueue(onNewChat, args)
);
...
eventSource.on(event_types.GENERATION_AFTER_COMMANDS, (...args) =>
  executeIfReadyElseQueue(onBeforeGeneration, args)
);
