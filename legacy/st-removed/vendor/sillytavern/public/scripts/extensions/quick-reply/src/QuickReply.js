...
export class QuickReply {
   ...
   */
  static from(props) {
    props.contextList = (props.contextList ?? []).map((/** @type {any} */ it) =>
      QuickReplyContextLink.from(it)
    );
  ...
  async showEditor() {
    const response = await fetch('/scripts/extensions/quick-reply/html/qrEditor.html', {
      cache: 'no-store',
    });
    if (response.ok) {
      this.template = document
        ...
        .querySelector('#qr--modalEditor');
      /**@type {HTMLElement} */
      // @ts-ignore
      const dom = this.template.cloneNode(true);
      this.editorDom = dom;
      this.editorPopup = new Popup(dom, POPUP_TYPE.TEXT, undefined, {
        okButton: 'OK',
        wide: true,
        large: true,
        rows: 1,
      ...
      dom.querySelector('#qr--modal-switcher').addEventListener('click', (evt) => {
        ...
        const list = document.createElement('ul');
        {
          ...
          const makeList = (qrs) => {
            ...
            for (const qr of qrs.qrList.toSorted((a, b) =>
      ...
      if (navigator.keyboard) {
        ...
        navigator.keyboard
          .getLayoutMap()
          .then(
            (it) => (dom.querySelector('#qr--modal-commentKey').textContent = it.get('Backslash'))
          );
      } else {
      ...
      message.addEventListener('keydown', async (evt) => {
        ...
        if (evt.key == 'Tab' && !evt.shiftKey && !evt.ctrlKey && !evt.altKey) {
          ...
          const start = message.selectionStart;
          const end = message.selectionEnd;
          const comment = parser.commandIndex.findLast(
            (it) =>
              it.name == '*' &&
              ((it.start <= start && it.end >= start) || (it.start <= end && it.end >= end))
          );
          if (comment) {
            // uncomment
            ...
            document.execCommand(
              'insertText',
              false,
              `/* ${message.value.slice(lineStart, lineEnd)} *|`
            );
      ...
      const updateSyntaxLoop = () => {
        ...
        if (wasSyntax == syntax.checked && lastMessageValue == message.value)
          return requestAnimationFrame(updateSyntaxLoop);
      ...
      const fillQrSetSelect = (
        /**@type {HTMLSelectElement}*/ select,
        /**@type {QuickReplyContextLink}*/ link
      ) => {
        [
          { name: 'Select a QR set' },
          ...QuickReplySet.list.toSorted((a, b) =>
      ...
      const inputOgMo = new MutationObserver((muts) => {
        if (
          muts.find(
            (it) =>
              [...it.removedNodes].includes(inputMirror) ||
              [...it.removedNodes].find((n) => n.contains(inputMirror))
          )
        ) {
    ...
    if (!this.clone) {
      ...
      const mo = new MutationObserver((muts) => {
        if (
          muts.find(
            (it) =>
              [...it.removedNodes].includes(this.editorMessage) ||
              [...it.removedNodes].find((n) => n.contains(this.editorMessage))
          )
        ) {
          this.clone?.remove();
          this.clone = null;
  ...
  async executeFromEditor() {
    ...
    const noSyntax = this.editorDom
      .querySelector('#qr--modal-messageHolder')
      .classList.contains('qr--noSyntax');
    if (noSyntax) {
    ...
    try {
      ...
      this.debugController.onBreakPoint = async (closure, executor) => {
        ...
        syntax.innerHTML = hljs.highlight(
          `${closure.fullText}${closure.fullText.slice(-1) == '\n' ? ' ' : ''}`,
          ...
          const wrap = document.createElement('div');
          {
            ...
            if (isCurrent) {
              const executor = this.debugController.cmdStack.slice(-1)[0];
              {
                ...
                const keys = new Set([
                  ...Object.keys(this.debugController.namedArguments ?? {}),
                  ...(executor.namedArgumentList ?? []).map((it) => it.name),
                ]);
              ...
              }
              {
                ...
                // @ts-ignore
                while (unnamed.length < executor.unnamedArgumentList?.length ?? 0)
                  unnamed.push(undefined);
                // @ts-ignore
                  ...
                  const item = document.createElement('div');
                  {
                    ...
                    const vResolved = document.createElement('div');
                    {
                      ...
                      if (this.debugController.unnamedArguments === undefined) {
                        ...
                        (Array.isArray(this.debugController.unnamedArguments)
                          ? this.debugController.unnamedArguments
                          : [this.debugController.unnamedArguments]
                        ).length < i
                      ) {
                        // do nothing
            ...
            const title = document.createElement('div');
            {
              ...
              if (c.source == source) {
                ...
                title.addEventListener('pointerenter', () => {
                  const loc = this.getEditorPosition(
                    Math.max(0, c.executorList[0].start - 1),
                    c.executorList.slice(-1)[0].end,
                    c.fullText
                  );
