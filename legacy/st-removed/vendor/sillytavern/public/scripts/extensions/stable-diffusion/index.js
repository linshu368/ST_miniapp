...
import { ActionLoaderHandle, loader } from '../../action-loader.js';
...
function getSdRequestBody() {
  switch (extension_settings.sd.source) {
    ...
    case sources.drawthings:
      return {
        url: extension_settings.sd.drawthings_url,
        auth: extension_settings.sd.drawthings_auth,
      };
    default:
...
async function loadSettings() {
  ...
  $('#sd_minimal_prompt_processing').prop(
    'checked',
    extension_settings.sd.minimal_prompt_processing
  );
...
function getClosestKnownResolution() {
  ...
  const diffs = Object.entries(resolutionOptions).map(([id, resolution]) => {
    ...
    const resolutionDiff =
      Math.abs(resolution.width * resolution.height - targetResolution) / targetResolution;
...
function addPromptTemplates() {
  ...
  for (const [name, prompt] of Object.entries(extension_settings.sd.prompts).sort(
    ...
    const container = $('<div></div>').addClass('title_restorable').append(label).append(button);
...
async function onDeleteStyleClick() {
  ...
  const confirmed = await callGenericPopup(
    t`Are you sure you want to delete the style "${selectedStyle}"?`,
    POPUP_TYPE.CONFIRM,
    '',
...
async function onChatChanged() {
  ...
  if (
    typeof sharedPromptData?.positive === 'string' &&
    !characterPrompt &&
    sharedPromptData.positive
  ) {
    characterPrompt = sharedPromptData.positive;
    extension_settings.sd.character_prompts[key] = characterPrompt;
  }
  if (
    typeof sharedPromptData?.negative === 'string' &&
    !negativePrompt &&
    sharedPromptData.negative
  ) {
...
const resolutionOptions = {
  ...
  sd_res_540x960: {
    width: 540,
    height: 960,
    name: translate('540x960 (9:16, vertical wallpaper)', 'sd_res_540x960'),
  },
  sd_res_1920x1088: {
    width: 1920,
    height: 1088,
...
async function onModelChange() {
  ...
  const updateRemoteModelSources = [sources.auto, sources.vlad, sources.extras];
  ...
  if (
    ...
    extension_settings.sd.source === sources.vlad
  ) {
...
async function loadWorkersAIImageModels() {
  ...
  if (!oai_settings.workers_ai_account_id) {
    toastr.warning(
      'Workers AI account ID is required. Save it in the "API Connections" panel.',
      'Image Generation'
    );
    return [];
  }
...
async function generatePicture(initiator, args, trigger, message, callback) {
  ...
  const generationTypeKey = Object.keys(generationMode).find(
    (key) => generationMode[key] === generationType
  );
...
function setTypeSpecificDimensions(generationType, mediaAttachment = null) {
  ...
  if (extension_settings.sd.snap) {
    ...
    if (prevPixelCount !== newPixelCount) {
      ...
      console.log(
        `Pixel counts after rescaling: ${prevPixelCount} -> ${newPixelCount} (ratio: ${ratio})`
      );
...
function generateFreeModePrompt(trigger, combineNegatives) {
  return trigger.replace(/^char(\s|,)|{{charPrefix}}/gi, (_, suffix) => {
    ...
    typeof combineNegatives === 'function' && negativeValue
      ? combineNegatives(negativeValue)
      : void 0;
...
async function generateMultimodalPrompt(generationType, quietPrompt) {
  ...
  if (
    generationType === generationMode.CHARACTER_MULTIMODAL ||
    generationType === generationMode.FACE_MULTIMODAL
  ) {
...
function getCharacterAvatarUrl() {
  ...
  if (context.groupId) {
    ...
    const lastMessageAvatar = context.chat
      ?.filter((x) => !x.is_system && !x.is_user)
...
async function sendGenerationRequest(
  ...
  const negativePrompt = substituteParams(
    combinePrefixes(additionalNegativePrefix, negativePrefix)
  );
...
async function getClosestSize(width, height, sizes = []) {
  ...
  const closestSize = sizesData.reduce(
    (closest, size) => {
    ...
    const response = await fetch('/api/sd/stability/generate', {
      ...
      body: JSON.stringify({
        ...
        payload: {
          ...
          aspect_ratio: getClosestAspectRatio(
...
async function generateAutoImage(prompt, negativePrompt, signal) {
  const isValidVae =
    extension_settings.sd.vae && !['N/A', placeholderVae].includes(extension_settings.sd.vae);
  let payload = {
    ...getSdRequestBody(),
    prompt: prompt,
    negative_prompt: negativePrompt,
...
function getNovelParams() {
  ...
  if (
    extension_settings.sd.sampler === 'ddim' ||
    ['nai-diffusion-4-curated-preview', 'nai-diffusion-4-full'].includes(
...
async function generateOpenAiImage(prompt, signal) {
  ...
  const result = await fetch('/api/openai/generate-image', {
    ...
    body: JSON.stringify({
      ...
      n: 1,
      quality: isDalle3
        ? extension_settings.sd.openai_quality
        : isGptImg
 ...
 * @returns {Promise<{format: string, data: string}>} - A promise that resolves when the image generation and processing are complete.
 */
async function generateComfyImageCommon(
  prompt,
  negativePrompt,
  signal,
  basePath,
  ...
  const seed =
    extension_settings.sd.seed >= 0
      ? extension_settings.sd.seed
      : Math.round(Math.random() * Number.MAX_SAFE_INTEGER);
  workflow = workflow.replaceAll('"%seed%"', JSON.stringify(seed));

  const denoising_strength =
    extension_settings.sd.denoising_strength === undefined
      ? 1.0
      : extension_settings.sd.denoising_strength;
...
async function generateComfyImage(prompt, negativePrompt, signal) {
  ...
  return generateComfyImageCommon(
    ...
    '/api/sd/comfy',
    placeholders,
    extension_settings.sd.comfy_url
  );
}
...
async function generateComfyRunPodImage(prompt, negativePrompt, signal) {
  ...
  return generateComfyImageCommon(
    ...
    '/api/sd/comfyrunpod',
    placeholders,
    extension_settings.sd.comfy_runpod_url
  );
}
...
async function generateXAIImage(prompt, _negativePrompt, signal) {
  ...
  if (/grok-imagine/.test(extension_settings.sd.model)) {
    ...
    const use2kResolution =
      extension_settings.sd.width * extension_settings.sd.height > resolutionThreshold;
    aspectRatio = getClosestAspectRatio(
      extension_settings.sd.width,
      extension_settings.sd.height,
      'xai'
    );
...
async function generateZaiImage(prompt, signal) {
  ...
  if (/(cogvideox|vidu)/.test(extension_settings.sd.model)) {
    ...
    if (/cogvideox/.test(extension_settings.sd.model)) {
      ...
      videoParams.size = await getClosestSize(
        extension_settings.sd.width,
        extension_settings.sd.height,
        cogVideoSizes
      );
    ...
    if (/vidu/.test(extension_settings.sd.model)) {
      videoParams.aspect_ratio = getClosestAspectRatio(
        extension_settings.sd.width,
        extension_settings.sd.height,
        'zai'
      );
...
async function onComfyOpenWorkflowEditorClick() {
  let workflow = await (
    await fetch('/api/sd/comfy/workflow', {
      method: 'POST',
      headers: getRequestHeaders(),
      body: JSON.stringify({
        file_name: extension_settings.sd.comfy_workflow,
  ...
  const addPlaceholderDom = (placeholder) => {
    ...
    el.find('.sd_comfy_workflow_editor_custom_remove').on('click', () => {
      el.remove();
      extension_settings.sd.comfy_placeholders.splice(
        extension_settings.sd.comfy_placeholders.indexOf(placeholder)
      );
      saveSettingsDebounced();
    });
  };
...
async function onComfyDeleteWorkflowClick() {
  const confirm = await callGenericPopup(
    t`Delete the workflow? This action is irreversible.`,
    POPUP_TYPE.CONFIRM,
    '',
...
async function onComfyRenameWorkflowClick() {
  ...
  const existingWorkflow = Array.from(document.querySelectorAll('#sd_comfy_workflow option')).find(
    (opt) => opt instanceof HTMLOptionElement && opt.value === newName
  );
...
async function addSDGenButtons() {
  ...
  $(document).on('click', '.sd_message_gen', (e) =>
    sdMessageButton($(e.currentTarget), { animate: false })
  );
...
async function sdMessageButton($icon, { animate } = {}) {
  ...
  const selectedMedia =
    message.extra.media.length > 0
      ? (message.extra.media[message.extra.media_index] ??
        message.extra.media[message.extra.media.length - 1])
      : {
          url: '',
          title: message.mes,
  ...
  if (animate && message.extra.media.length > 0) {
    ...
    $media = messageElement
      .find(`.mes_media_container[data-index="${index}"]`)
      .find('.mes_img, .mes_video');
  }
...
async function writePromptFields(characterId) {
  ...
  const negativePromptPrefix = key
    ? extension_settings.sd.character_negative_prompts[key] || ''
    : '';
  const promptObject = {
...
async function generateMediaSwipe(
  ...
  const generationType =
    mediaAttachment.generation_type ?? message?.extra?.generationType ?? generationMode.FREE;
    ...
    const refineArgs = {
      ...
      resolution:
        mediaAttachment.width && mediaAttachment.height
          ? `${mediaAttachment.width}x${mediaAttachment.height}`
          : null,
    };
    const prompt = await refinePrompt(savedPrompt, refineArgs);
    dimensions = setTypeSpecificDimensions(
      generationType,
...
async function onImageSwiped({ message, element, direction }) {
  ...
  if (
    ...
    powerUserSettings.image_overswipe !== IMAGE_OVERSWIPE.GENERATE
  ) {
    return;
  }
    ...
    parameters: Object.freeze({
      ...
      properties: {
        prompt: {
          type: 'string',
          description:
            extension_settings.sd.prompts[generationMode.TOOL] ||
...
export async function init() {
    ...
    SlashCommand.fromProps({
      ...
      callback: async (args, trigger) => {
        ...
        try {
          ...
          if (
            !isTrueBoolean(args?.quiet?.toString()) &&
      ...
      namedArgumentList: [
        ...
        new SlashCommandNamedArgument(
          'gallery',
          'whether to save the generated image to the character gallery',
          [ARGUMENT_TYPE.BOOLEAN],
          false,
          false,
          'true'
        ...
        SlashCommandNamedArgument.fromProps({
          name: 'snap',
          description:
            'snap auto-adjusted dimensions to the nearest known resolution (portraits and backgrounds only)',
          typeList: [ARGUMENT_TYPE.BOOLEAN],
          enumProvider: commonEnumProviders.boolean('trueFalse'),
          isRequired: false,
          acceptsMultiple: false,
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      helpString:
        '(workflowName) - change the workflow to be used for image generation with ComfyUI, e.g. <pre><code>/imagine-comfy-workflow MyWorkflow</code></pre>',
    })
  );
  ...
  [event_types.SECRET_WRITTEN, event_types.SECRET_DELETED, event_types.SECRET_ROTATED].forEach(
    (event) => {
      eventSource.on(event, async (/** @type {string} */ key) => {
        ...
        const shouldReloadOptions = Object.entries(keySourceMap).some(
          ([k, v]) => k === extension_settings.sd.source && v === key
        );
