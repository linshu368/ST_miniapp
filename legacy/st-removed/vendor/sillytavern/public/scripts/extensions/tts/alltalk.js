...
<span>- <strong>Text-generation-webui</strong> users - Uncheck <strong>Enable TTS</strong> in the TGWUI interface, or you will hear 2x voices and file names being generated.</span>
  ...
  updateCheckboxes() {
    ...
    if (deepspeedCheckbox) {
      if (this.settings.deepspeed_capable) {
        ...
        this.settings.deepspeed_enabled =
          this.settings.deepspeed_available && this.settings.deepspeed_enabled;
      } else {
  ...
  setupEventListeners() {
    ...
    const onModelSelectChange = async (event) => {
      ...
      try {
        const response = await fetch(
          `${this.settings.provider_endpoint}/api/reload?tts_method=${encodeURIComponent(selectedModel)}`,
          {
            method: 'POST',
          }
        );
    ...
    if (deepspeedCheckbox) {
      const handleDeepSpeedChange = async (event) => {
        ...
        try {
          const response = await fetch(
            `${this.settings.provider_endpoint}/api/deepspeed?new_deepspeed_value=${deepSpeedValue}`,
            {
              method: 'POST',
            }
          );
    ...
    if (lowVramCheckbox) {
      const handleLowVramChange = async (event) => {
        ...
        try {
          const response = await fetch(
            `${this.settings.provider_endpoint}/api/lowvramsetting?new_low_vram_value=${lowVramValue}`,
            {
              method: 'POST',
            }
          );
  ...
  async previewTtsVoice(voiceName) {
    try {
      ...
      if (
        this.settings.server_version === 'v2' &&
        this.settings.rvc_character_voice !== 'Disabled'
      ) {
      ...
      if (data.output_file_url) {
        ...
        const fullUrl =
          this.settings.server_version === 'v1'
  ...
  async fetchTtsGeneration(inputText, voiceId) {
    ...
    try {
      const response = await doExtrasFetch(`${this.settings.provider_endpoint}/api/tts-generate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
