...
class ElevenLabsTtsProvider {
  ...
  onSettingsChange() {
    ...
    $('#elevenlabs_tts_similarity_boost_output').text(
      Math.round(this.settings.similarity_boost * 100) + '%'
    );
    $('#elevenlabs_tts_style_exaggeration_output').text(
      Math.round(this.settings.style_exaggeration * 100) + '%'
    );
  ...
  setupVoiceCloningMenu() {
    const audioFilesInput = /** @type {HTMLInputElement} */ (
      document.getElementById('elevenlabs_tts_audio_files')
    );
    ...
    const voiceCloningNameInput = /** @type {HTMLInputElement} */ (
      document.getElementById('elevenlabs_tts_voice_cloning_name')
    );
    const voiceCloningDescriptionInput = /** @type {HTMLInputElement} */ (
      document.getElementById('elevenlabs_tts_voice_cloning_description')
    );
    const voiceCloningLabelsInput = /** @type {HTMLInputElement} */ (
      document.getElementById('elevenlabs_tts_voice_cloning_labels')
    );
    ...
    cloneVoiceButton.addEventListener('click', async () => {
      ...
      try {
        ...
        toastr.success(
          'Voice cloned successfully. Hit reload to see the new voice in the voice listing.'
        );
        clearSelectedFiles();
