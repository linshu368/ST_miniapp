...
var speechUtteranceChunker = function (utt, settings, callback) {
  ...
  var newUtt;
  var txt =
    settings && settings.offset !== undefined ? utt.text.substring(settings.offset) : utt.text;
...
class SystemTtsProvider {
  ...
  get settingsHtml() {
    ...
    return (
      '<p>' +
      t`Uses the voices provided by your operating system` +
      `</p>
        <label for="system_tts_rate">` +
      t`Rate:` +
      ` <span id="system_tts_rate_output"></span></label>
        <input id="system_tts_rate" type="range" value="${this.defaultSettings.rate}" min="0.1" max="2" step="0.01" />
        <label for="system_tts_pitch">` +
      t`Pitch:` +
      ` <span id="system_tts_pitch_output"></span></label>
