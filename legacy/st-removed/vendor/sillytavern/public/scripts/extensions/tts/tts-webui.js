...
class TtsWebuiProvider {
  ...
  onSettingsChange() {
    ...
    this.settings.remove_milliseconds_start = Number(
      $('#tts_webui_remove_milliseconds_start').val()
    );
  ...
  async fetchTtsVoiceObjects() {
    ...
    try {
      const voicesEndpoint = this.settings.provider_endpoint.replace(
        '/speech',
        '/voices/' + this.settings.model
      );
  ...
  async fetchTtsGeneration(inputText, voiceId) {
    ...
    const getParams = (settings) =>
      Object.fromEntries(
        Object.entries(settings).filter(([key]) => chatterboxParams.includes(key))
      );
