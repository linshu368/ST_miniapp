...
class VolcengineTtsProvider {
  ...
  constructor() {
    this.handler = async function (/** @type {string} */ key) {
      ...
      $('#volcengine-tts-app-id').toggleClass(
        'success',
        !!secret_state[SECRET_KEYS.VOLCENGINE_APP_ID]
      );
      $('#volcengine-tts-access-key').toggleClass(
        'success',
        !!secret_state[SECRET_KEYS.VOLCENGINE_ACCESS_KEY]
      );
  ...
  async loadSettings(settings) {
    ...
    $('#volcengine-tts-resource-id')
      .val(this.settings.resource_id)
    ...
    $('#volcengine-tts-provider-endpoint')
  ...
  async deleteSelectedVoice() {
    ...
    const confirm = await Popup.show.confirm(
      `Are you sure you want to delete the selected voice ${selectedVoiceName}?`
    );
    if (!confirm) {
      return;
    }
