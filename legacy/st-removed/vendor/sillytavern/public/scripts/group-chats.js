...
export function getGroupCharacterCardsLazy(groupId, characterId) {
  ...
  return createLazyFields({
    ...
    scenario: () =>
      baseChatReplace(scenarioOverride?.trim()) || collectField('Scenario', (c) => c.scenario),
    mesExamples: () =>
...
function getGroupAvatar(group) {
  ...
  if (isValidImageUrl(group.avatar_url)) {
    return $(
      `<div class="avatar" title="[Group] ${group.name}"><img src="${group.avatar_url}"></div>`
    );
  }
...
function activatePooledOrder(members, lastMessage, isUserInput) {
  ...
  if (activatedMember === null) {
    const lastMessageAvatar =
      members.length > 1 && lastMessage && !lastMessage.is_user && lastMessage.original_avatar;
...
async function createGroup() {
  ...
  let activationStrategy =
    Number($('#rm_group_activation_strategy').find(':selected').val()) ??
    group_activation_strategy.NATURAL;
  let generationMode =
    Number($('#rm_group_generation_mode').find(':selected').val()) ?? group_generation_mode.SWAP;
