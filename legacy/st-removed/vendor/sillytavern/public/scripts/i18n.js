...
async function getMissingTranslations() {
          ...
          if (attributeMatch) {
            ...
            if (!localizedValue) {
              missingData.push({
                ...
                value: String($(this).attr(attributeMatch[1])),
              });
            }
          } else {
            // No attribute tag, treat as 'text'
  ...
  for (const { key, language, value } of missingData) {
    if (
      !uniqueMissingData.some((x) => x.key === key && x.language === language && x.value === value)
    ) {
...
export function applyLocale(root = document) {
  ...
  const $root =
    root instanceof Document ? $(root) : $(new DOMParser().parseFromString(root, 'text/html'));
