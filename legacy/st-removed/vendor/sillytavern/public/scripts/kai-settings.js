  ...
  let generate_data = {
    ...
    use_default_badwordsids:
      kai_flags.can_use_default_badwordsids || isHorde
        ...
        : undefined,
    grammar:
      kai_flags.can_use_grammar || isHorde ? substituteParams(kai_settings.grammar) : undefined,
...
export function initKoboldSettings() {
  ...
  $('#settings_preset').on('change', async function () {
    if ($('#settings_preset').find(':selected').val() != 'gui') {
      ...
      $('#kobold_order').css('opacity', 1).sortable('enable');
    } else {
      kai_settings.preset_settings = 'gui';
      ...
      $('#kobold_order').css('opacity', 0.5).sortable('disable');
    }
    saveSettingsDebounced();
