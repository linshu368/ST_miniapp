...
export class MacrosParser {
    ...
    macroSystem.registry.registerMacro(key, {
      ...
      description:
        typeof description === 'string'
          ? description
          : 'Automatically registered macro from MacrosParser',
      handler: () => {
  ...
  static get(key) {
    MacrosParser.#logDeprecated(
      'get',
      'macros.registry.getMacro (from scripts/macros/macro-system.js)',
      arguments
    );
  ...
  static has(key) {
    MacrosParser.#logDeprecated(
      'has',
      'macros.registry.hasMacro (from scripts/macros/macro-system.js)',
      arguments
    );
    if (power_user.experimental_macro_engine) {
...
export function evaluateMacros(content, env, postProcessFn) {
  ...
  const preEnvMacros = [
    ...
    { regex: /<CHAR>/gi, replace: () => (typeof env.char === 'function' ? env.char() : env.char) },
    {
      ...
      replace: () => (typeof env.group === 'function' ? env.group() : env.group),
    },
    {
      regex: /<GROUP>/gi,
      replace: () => (typeof env.group === 'function' ? env.group() : env.group),
  ...
  const postEnvMacros = [
    ...
    { regex: /{{currentSwipeId}}/gi, replace: () => String(getCurrentSwipeId() ?? '') },
    {
      ...
      replace: () => (chat.length === 0 ? '' : `0-${chat.length - 1}`),
    },
