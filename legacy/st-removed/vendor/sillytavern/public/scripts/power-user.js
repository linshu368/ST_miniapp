...
import {
  ...
  deriveBackgroundName,
} from './util/ThemeGenerator.js';
...
async function switchLabMode({ noReset = false } = {}) {
  ...
  if (power_user.enableLabMode) {
    ...
    $('#amount_gen_counter').attr('min', '1').attr('max', '99999').attr('step', '1');
    $('#amount_gen').attr('min', '1').attr('max', '99999').attr('step', '1');
  } else if (!noReset) {
            ...
            if (isManualInput) {
              ...
              if (manualInput >= sliderMin && manualInput <= sliderMax) {
                ...
                handleSlideEvent.call(
                  newSlider,
                  null,
                  { value: parseFloat(valueBeforeManualInput) },
                  'manual'
                );
              }
            }
...
export async function loadPowerUserSettings(settings, data) {
  ...
  if (settings.power_user !== undefined) {
    ...
    if (
      Object.hasOwn(settings.power_user, 'auto_sort_tags') &&
      !Object.hasOwn(settings.power_user, 'tag_sort_mode')
    ) {
      settings.power_user.tag_sort_mode = settings.power_user.auto_sort_tags
    ...
    if (power_user.stscript.autocomplete === undefined) {
      ...
      if (power_user.stscript.autocomplete.style === undefined) {
        power_user.stscript.autocomplete.style =
          power_user.stscript.autocomplete_style || defaultStscript.autocomplete.style;
      }
  ...
  $(`#example_messages_behavior option[value="${getExampleMessagesBehavior()}"]`).prop(
    'selected',
    true
  );
  $('#instruct_derived')
    .parent()
    .find('i')
    .toggleClass('toggleEnabled', !!power_user.instruct_derived);
  $('#context_derived')
    .parent()
    .find('i')
  ...
  $('#confirm_message_delete').prop(
    'checked',
    power_user.confirm_message_delete !== undefined ? !!power_user.confirm_message_delete : true
  );
  ...
  $('#stscript_autocomplete_showInAllMacroFields')
    .prop('checked', power_user.stscript.autocomplete.showInAllMacroFields ?? false)
    .trigger('input');
...
function validateStoryString(storyString, params) {
  ...
  const cache = JSON.parse(accountStorage.getItem(storage_keys.storyStringValidationCache)) ?? {
    hashCache: {},
  };
...
export function sortEntitiesList(entities, forceSearch, filterHelper = null) {
  ...
  const isSearch =
    forceSearch || $('#character_sort_order option[data-field="search"]').is(':selected');
...
jQuery(() => {
  ...
  $('#stscript_autocomplete_width_left').on('input', function () {
    ...
    /**@type {HTMLElement}*/ (this.closest('.doubleRangeInputContainer')).style.setProperty(
      '--value',
      value.toString()
    );
    window.dispatchEvent(new Event('resize', { bubbles: true }));
    ...
    SlashCommand.fromProps({
      ...
      unnamedArgumentList: [
        SlashCommandArgument.fromProps({
          ...
          enumProvider: () =>
            tags
              .filter((tag) => Object.values(tag_map).some((x) => x.includes(tag.id)))
              .map(
                (tag) => new SlashCommandEnumValue(tag.name, null, enumTypes.enum, enumIcons.tag)
              ),
        }),
      ],
      helpString:
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      helpString:
        'Generates a new theme based on a dominant color of the specified background image. Saves as "bgcol - background name".',
    })
  );
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
    ...
    SlashCommand.fromProps({
      ...
      unnamedArgumentList: [
        SlashCommandArgument.fromProps({
          ...
          isRequired: true,
          enumProvider: () =>
            movingUIPresets.map((preset) => new SlashCommandEnumValue(preset.name)),
        }),
      ],
