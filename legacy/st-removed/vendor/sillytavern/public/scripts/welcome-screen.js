      ...
      pinButton.addEventListener('click', async (event) => {
        ...
        const recentChat = chats.find(
          (c) =>
            ...
            ((c.is_group && c.group === groupId) || (!c.is_group && c.avatar === avatarId))
        );
        if (!recentChat) {
    ...
    if (chatToFlash instanceof HTMLElement) {
      if (!isElementInViewport(chatToFlash)) {
        chatElement.scrollTop =
          chatToFlash.offsetTop - chatElement.offsetTop - chatToFlash.clientHeight / 2;
      }
...
async function openRecentChatsSettingsPopup() {
  ...
  await callGenericPopup(t`Recent Chats Settings`, POPUP_TYPE.CONFIRM, null, {
    ...
    onClose: (popup) => {
      ...
      const maxInputValue =
        popup.inputResults.get(maxRecentChatsInput.id)?.toString() ?? String(DEFAULT_MAX_DISPLAYED);
