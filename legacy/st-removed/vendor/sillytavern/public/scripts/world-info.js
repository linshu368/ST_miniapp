...
export function setWorldInfoSettings(settings, data) {
  ...
  $(`#world_info_character_strategy option[value='${world_info_character_strategy}']`).prop(
    'selected',
    true
  );
  ...
  eventSource.on(event_types.CHAT_CHANGED, async () => {
    const hasWorldInfo =
      !!chat_metadata[METADATA_KEY] && world_names.includes(chat_metadata[METADATA_KEY]);
...
function registerWorldInfoSlashCommands() {
  ...
  async function getCharBookCallback({ type, name, create }, characterIdentifier) {
    ...
    if (context.groupId && !characterIdentifier)
      throw new Error('This command is not available in groups without providing a character name');
    type =
      String(type ?? '')
        .trim()
        .toLowerCase() || 'primary';
    characterIdentifier =
      String(characterIdentifier ?? '') || context.characters[context.characterId]?.avatar || null;
  ...
  async function setEntryFieldCallback(args, value) {
    ...
    if (!data || !('entries' in data)) {
      toastr.warning('Valid World Info file name is required');
      logSlashCommandWarn('setEntryFieldCallback: Valid World Info file name is required', args, {
        value,
      });
      return '';
    }
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      unnamedArgumentList: [new SlashCommandArgument('texts', ARGUMENT_TYPE.STRING, true, true)],
      helpString: `
            <div>
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      unnamedArgumentList: [new SlashCommandArgument('content', [ARGUMENT_TYPE.STRING], false)],
      helpString: `
            <div>
...
async function displayWorldEntries(
  ...
  $('#world_info_pagination').pagination({
    ...
    callback: async function (/** @type {object[]} */ page) {
      try {
        ...
        const isCustomOrder =
          $('#world_info_sort_order').find(':selected').data('rule') === 'custom';
        if (!isCustomOrder) {
  ...
  $('#world_apply_current_sorting')
    ...
    .on('click', async () => {
      ...
      let content =
        '<span>' +
        t`Apply your current sorting to the "Order" field. The Order values will go down from the chosen number.` +
        '</span>';
      if (moreThan100) {
        content +=
          '<div class="m-t-1"><i class="fa-solid fa-triangle-exclamation" style="color: #FFD43B;"></i> ' +
          t`More than 100 entries in this world. If you don't choose a number higher than that, the lower entries will default to 0.<br />(Usual default: 100)<br />Minimum: ${entryCount}` +
          '</div>';
      }
      ...
      if (start < entryCount) {
        toastr.warning(
          t`A number lower than the entry count has been chosen. All entries below that will default to 0.`,
    ...
    stop: async function (_event, _ui) {
      ...
      console.table(
        Object.keys(data.entries)
          ...
          .map((x) => ({ uid: x.uid, key: x.key.join(','), displayIndex: x.displayIndex }))
      );
...
function handleEntryStateSelectorHelper({ entryStateSelector, entry, data, name }) {
  ...
  const entryState = () =>
    entry.constant === true ? 'constant' : entry.vectorized === true ? 'vectorized' : 'normal';
  entryStateSelector
  ...
  function addEditorDrawerContent() {
    ...
    editTemplate
      ...
      .each((_, icon) => {
        $(icon).attr(
          'title',
          $(icon).data(power_user.wi_key_input_plaintext ? 'tooltip-on' : 'tooltip-off')
        );
    ...
    characterFilterLabel.text(
      entry.characterFilter?.isExclude ? 'Exclude Character(s)' : 'Filter to Character(s)'
    );
    ...
    setTimeout(
      () =>
        createEntryInputAutocomplete(outletNameInput, getOutletNameCallback(data), {
          allowMultiple: true,
        }),
      1
    );
    ...
    setTimeout(
      () =>
        createEntryInputAutocomplete(groupInput, getInclusionGroupCallback(data), {
          allowMultiple: true,
        }),
      1
    );
    ...
    setTimeout(
      () => createEntryInputAutocomplete(automationIdInput, getAutomationIdCallback(data)),
      1
    );
...
function getOutletNameCallback(data) {
  return buildAutocompleteCallback({
    data,
    collectValues: (entry) =>
      entry.position === world_info_position.outlet && entry.outletName ? [entry.outletName] : [],
  });
}
...
function createEntryInputAutocomplete(input, callback, { allowMultiple = false } = {}) {
  ...
  $(input).on('focus click', function () {
    $(input).autocomplete(
      'search',
      allowMultiple ? String($(input).val()).split(/,\s*/).pop() : String($(input).val())
    );
  });
}
...
export async function checkWorldInfo(
  ...
  while (scanState) {
    ...
    const minActivationsNotSatisfied =
      world_info_min_activations > 0 && allActivatedEntries.size < world_info_min_activations;
...
export async function importEmbeddedWorldInfo(skipPopup = false) {
  ...
  const bookName =
    characters[chid]?.data?.character_book?.name || `${characters[chid]?.name}'s Lorebook`;
