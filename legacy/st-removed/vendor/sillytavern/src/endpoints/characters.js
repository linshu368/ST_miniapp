...
async function importFromByaf(uploadPath, { request }, preservedFileName) {
  ...
  if (!preservedFileName) {
    ...
    for (const bg of byafData.chatBackgrounds) {
      ...
      const file = getUniqueName(baseName, (name) =>
...
router.post(
  ...
  async function (request, response) {
    try {
      ...
      const internalName =
        request.body.file_name || getPngName(request.body.ch_name, request.user.directories);
  ...
  async function (request, response) {
    try {
      ...
      if (result.ok) {
        ...
        response
          .status(400)
          .send({ message: `Validation failed for ${update.avatar}`, error: result.error });
      }
    } catch (exception) {
      response
        .status(500)
        .send({ message: 'Unexpected error while saving character.', error: exception.toString() });
    }
  }
);
