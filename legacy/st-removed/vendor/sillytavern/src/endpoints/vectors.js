...
async function getBatchVector(source, sourceSettings, texts, isQuery, directories) {
  ...
  for (let batch of batches) {
    switch (source) {
      ...
      case 'openai':
        results.push(
          ...(await getOpenAIBatchVector(batch, source, directories, sourceSettings.model))
        );
        break;
      case 'electronhub':
        results.push(
      ...
      case 'extras':
        results.push(
          ...(await getExtrasBatchVector(batch, sourceSettings.extrasUrl, sourceSettings.extrasKey))
        );
        break;
      case 'palm':
        results.push(
          ...(await getMakerSuiteBatchVector(batch, sourceSettings.model, sourceSettings.request))
        );
        break;
      case 'vertexai':
        results.push(
          ...(await getVertexBatchVector(batch, sourceSettings.model, sourceSettings.request))
        );
        break;
      case 'cohere':
        results.push(
          ...(await getCohereBatchVector(batch, isQuery, directories, sourceSettings.model))
        );
        break;
      case 'llamacpp':
  ...
  switch (source) {
    ...
    case 'siliconflow':
      return {
        ...
        urlOverride:
          request.body.siliconflow_endpoint === 'cn' ? 'https://api.siliconflow.cn/v1' : null,
      };
    case 'workers_ai': {
