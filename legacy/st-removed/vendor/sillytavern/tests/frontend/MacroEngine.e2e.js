...
test.describe('MacroEngine', () => {
  ...
  test.describe('Arity errors', () => {
    ...
    test('should not resolve reverse when called without arguments', async ({ page }) => {
      ...
      expect(
        warnings.some((w) => w.includes('Macro "reverse"') && w.includes('unnamed arguments'))
      ).toBeTruthy();
    });
  ...
  test.describe('Type validation', () => {
    test('should not resolve strict typed macro when argument type is invalid', async ({
      ...
      await page.evaluate(async () => {
        ...
        MacroRegistry.registerMacro('test-int-strict', {
          unnamedArgs: [{ name: 'value', type: 'integer', description: 'Must be an integer.' }],
          strictArgs: true,
      ...
      expect(
        warnings.some(
          (w) => w.includes('Macro "test-int-strict"') && w.includes('expected type integer')
        )
      ).toBeTruthy();
    });
    ...
    test('should resolve non-strict typed macro when argument type is invalid but still log warning', async ({
      ...
      expect(
        warnings.some(
          (w) => w.includes('Macro "test-int-nonstrict"') && w.includes('expected type integer')
        )
      ).toBeTruthy();
    });
  });
  ...
  test.describe('Dynamic macros', () => {
    test.describe('String value dynamic macros', () => {
      ...
      test('should not resolve string dynamic macro when called with arguments', async ({
        ...
        expect(output).toBe(input);
        expect(
          warnings.some((w) => w.includes('Macro "myvalue"') && w.includes('unnamed arguments'))
        ).toBeTruthy();
      });
    });
    ...
    test.describe('Handler function dynamic macros', () => {
      ...
      test('should not resolve handler dynamic macro when called with arguments due to strict arity', async ({
        ...
        expect(output).toBe(input);
        expect(
          warnings.some((w) => w.includes('Macro "dyn"') && w.includes('unnamed arguments'))
        ).toBeTruthy();
      });
    });
    ...
    test.describe('MacroDefinitionOptions dynamic macros', () => {
      ...
      test('should support multiple unnamed arguments in dynamic macro', async ({ page }) => {
        const output = await page.evaluate(async () => {
          ...
          const rawEnv = {
            ...
            dynamicMacros: {
              wrap: {
                unnamedArgs: [{ name: 'content' }, { name: 'prefix' }, { name: 'suffix' }],
      ...
      test('should handle invalid MacroDefinitionOptions gracefully', async ({ page }) => {
        ...
        expect(output).toBe(input);
        expect(
          warnings.some((w) => w.includes('bad') && w.includes('is not defined correctly'))
        ).toBeTruthy();
      });
    });
  ...
  test.describe('Variable Shorthand Syntax', () => {
    ...
    test('should add to local variable with += shorthand', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{.myvar += 3}}Then: {{.myvar}}', {
        local: { myvar: '7' },
      });
    ...
    test('should handle whitespace in variable shorthand', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{ .myvar = spaced }}{{.myvar}}', {
        local: {},
      });
    ...
    test('should return default value with || when variable does not exist', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{.nonexistent || default}}', {
        local: {},
      });
    ...
    test('should set and return default with ||= when variable is falsy', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{.myvar ||= newval}}{{.myvar}}', {
        local: { myvar: '' },
      });
    ...
    test('should use || with global variable', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{$myvar || globaldefault}}', {
        global: { myvar: '' },
      });
    ...
    test('should use ||= with global variable', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{$myvar ||= gset}}{{$myvar}}', {
        global: { myvar: '' },
      });
    ...
    test('should support nested macro in || fallback value', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{.myvar || Hello {{user}}}}', {
        local: {},
      });
      expect(output).toBe('Hello User');
    });
  ...
  test.describe('Variable Shorthand in {{if}} Macro', () => {
    ...
    test('should evaluate truthy global variable in if condition', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{if $enabled}}Active{{/if}}', {
        global: { enabled: 'true' },
      });
    ...
    test('should evaluate inverted variable condition', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{if !.flag}}Not set{{/if}}', {
        local: { flag: '' },
      });
    ...
    test('should evaluate inverted global variable condition', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{if !$disabled}}Enabled{{/if}}', {
        global: { disabled: '' },
      });
    ...
    test('should evaluate inverted condition with whitespace after !', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{if ! .empty}}Empty{{/if}}', {
        local: { empty: '' },
      });
      expect(output).toBe('Empty');
    });
    ...
    test('should handle variable with hyphen in if condition', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{if .is-valid}}Valid{{/if}}', {
        local: { 'is-valid': '1' },
      });
  ...
  test.describe('Variable Macros (hasvar, deletevar)', () => {
    ...
    test('should return false when global variable does not exist', async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, '{{hasglobalvar::nonexistent}}', {
        global: {},
      });
      expect(output).toBe('false');
    });
