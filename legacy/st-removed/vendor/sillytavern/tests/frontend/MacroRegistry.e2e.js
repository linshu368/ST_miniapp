    ...
    test.describe('valid', () => {
      ...
      test('should register alias with visible=false option', async ({ page }) => {
        const result = await page.evaluate(async () => {
          ...
          const success = MacroRegistry.registerMacroAlias('alias-target-hidden', 'hidden-alias', {
            visible: false,
          });
    ...
    test.describe('warnings', () => {
      test('should warn when alias overwrites existing macro', async ({ page }) => {
        ...
        const overwriteWarning = result.warnings.find(
          (w) => w.includes('overwrites an existing macro') && w.includes('overwrite-existing')
        );
      ...
      test('should warn when alias overwrites another alias', async ({ page }) => {
        ...
        const overwriteWarning = result.warnings.find(
          (w) => w.includes('overwrites an existing macro') && w.includes('shared-alias-name')
        );
        expect(overwriteWarning).toBeTruthy();
      });
    });
