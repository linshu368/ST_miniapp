...
test.describe('MacrosParser (legacy shim)', () => {
  ...
  test('should resolve macros via legacy evaluateMacros when experimental engine is disabled', async ({
    page,
  }) => {
  ...
  test('should resolve macros via new engine when experimental engine is enabled', async ({
    page,
  }) => {
    const output = await page.evaluate(async () => {
