...
> 对外域名绑 **Vercel**，前端作为边缘入口；网关收敛后 Railway 只跑 `backend` 一个服务，
> 它同时是唯一对外服务（拓扑见 [`railway/README.md`](./railway/README.md)）。
...
- `st-miniapp-st-backend`（st-bundle，ST + provision-api + watcher）随 ST 整包退场，
  `st-miniapp-nginx`（单域名网关）随网关收敛退场，两者的 Dockerfile 与镜像定义都已删除
  （取回见 commit `c737294` 及其之前的历史）。

---

## backend (M2)

`@miniapp/backend` — a single Fastify process (HTTP API)。See
...
| `UPSTASH_REDIS_REST_URL`                           | Config store    | unset                                        | Upstash Redis REST URL (runtime config).                                              |
| `UPSTASH_REDIS_REST_TOKEN`                         | Config store    | unset                                        | Upstash Redis REST token.                                                             |
...
- Real Supabase credential wiring is M4, not validated here.
