...
-- ==================== miniapp.tf_set_user_character_round() ====================
...
CREATE OR REPLACE FUNCTION miniapp_traffic.increment_click(p_source_id text)
 ...
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'miniapp_traffic', 'public'
AS $function$
...
-- ==================== public.compute_daily_metrics(p_from date, p_to date) ====================
CREATE OR REPLACE FUNCTION public.compute_daily_metrics(p_from date, p_to date)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
  ...
  -- 清除旧数据（幂等）
  DELETE FROM miniapp_analytics.session_metrics_daily
  WHERE stat_date BETWEEN p_from AND p_to;

  -- ====== 物化中间结果到临时表（函数结束自动清理）======

  -- 层1：标记 session 切割点（30min 间隔 → 新 session）
  CREATE TEMP TABLE tmp_marked AS
  SELECT
    user_id,
    created_at,
    (created_at AT TIME ZONE 'Asia/Shanghai')::DATE AS local_date,
    CASE
  ...
  FROM miniapp.chat_history
  WHERE created_at < (p_to + 1)::timestamptz;

  -- 层2：累加 is_new_session 得 session_seq
  CREATE TEMP TABLE tmp_sess_id AS
  SELECT
    user_id,
    created_at,
    local_date,
  ...
  -- 层3：每个 session 的轮次 + 时长
  CREATE TEMP TABLE tmp_sess AS
  SELECT
    user_id,
    session_seq,
    COUNT(*)::NUMERIC      AS rounds,
    MIN(local_date)        AS session_date,
    EXTRACT(EPOCH FROM (MAX(created_at) - MIN(created_at))) / 60.0::NUMERIC AS duration_min
  FROM tmp_sess_id
  ...
  SELECT
    ...
    SUM(rounds)                 AS n_rounds,
    MIN(session_date)           AS first_date
  FROM tmp_sess
  GROUP BY user_id;

  -- 层5：老用户（≥3 体验 且 总轮次 ≥ 3）
  CREATE TEMP TABLE tmp_old_u AS
  SELECT user_id FROM tmp_usr WHERE n_sessions >= 3 AND n_rounds >= 3;

  -- 层6：老用户的 session
  CREATE TEMP TABLE tmp_old_s AS
  SELECT s.* FROM tmp_sess s JOIN tmp_old_u o ON s.user_id = o.user_id;

  -- 创建索引加速逐日查询
  CREATE INDEX ON tmp_old_s(session_date);
  CREATE INDEX ON tmp_usr(first_date);

  -- ====== 批量写入 3 个指标（当天瞬时口径 A）======
  -- P50 只取当天当日的老用户 session，不累积历史

  -- 指标2：单次 session 轮次 P50（当天）
  INSERT INTO miniapp_analytics.session_metrics_daily (metric_name, stat_date, value)
  SELECT 'session_rounds_p50', d.d::DATE,
    COALESCE(ROUND((
      SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY rounds)
      FROM tmp_old_s WHERE session_date = d.d::DATE
    )::NUMERIC, 0), 0)
  FROM generate_series(p_from, p_to, INTERVAL '1 day') AS d(d);

  -- 指标3：单次 session 时长 P50（当天，分钟）
  INSERT INTO miniapp_analytics.session_metrics_daily (metric_name, stat_date, value)
  SELECT 'duration_p50', d.d::DATE,
    COALESCE(ROUND((
      SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY duration_min)
      FROM tmp_old_s WHERE session_date = d.d::DATE
    )::NUMERIC, 1), 0.0)
  FROM generate_series(p_from, p_to, INTERVAL '1 day') AS d(d);

  -- 指标1：D7 新转老转化率（滚动7天窗口：[d-7, d]）
  -- 逻辑：以当天d为基准，往前7天的用户中，达到老用户标准的比例
  -- 每天显示 = 以当天为基准向前推7天；第二天以第二天为基准向前推7天
  INSERT INTO miniapp_analytics.session_metrics_daily (metric_name, stat_date, value)
  SELECT 'd7_conversion_rate', d.d::DATE,
    COALESCE(ROUND(
      100.0 * (
        SELECT COUNT(DISTINCT u.user_id)::NUMERIC
        FROM tmp_usr u
        WHERE u.first_date BETWEEN (d.d::DATE - 7) AND d.d::DATE
          AND u.n_sessions >= 3
          AND u.user_id IN (SELECT user_id FROM tmp_old_u)
      ) / NULLIF(
        (SELECT COUNT(DISTINCT user_id) FROM tmp_usr
         WHERE first_date BETWEEN (d.d::DATE - 7) AND d.d::DATE),
        0::BIGINT
      ), 2), 0.0)
  FROM generate_series(p_from, p_to, INTERVAL '1 day') AS d(d);

  -- 清理临时表（可选，函数结束自动清理）
  DROP TABLE tmp_marked;
  DROP TABLE tmp_sess_id;
  DROP TABLE tmp_sess;
  DROP TABLE tmp_usr;
  DROP TABLE tmp_old_u;
  DROP TABLE tmp_old_s;
...
CREATE OR REPLACE FUNCTION public.show_dashboard()
 RETURNS TABLE("统计日期" date, "D7新转老转化率%" numeric, "单次session轮次P50" numeric, "单次session时长P50(分钟)" numeric)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY
  SELECT
    s.stat_date                                                            AS "统计日期",
    MAX(s.value) FILTER (WHERE s.metric_name = 'd7_conversion_rate')       AS "D7新转老转化率%",
    MAX(s.value) FILTER (WHERE s.metric_name = 'session_rounds_p50')       AS "单次session轮次P50",
    MAX(s.value) FILTER (WHERE s.metric_name = 'duration_p50')             AS "单次session时长P50(分钟)"
  FROM miniapp_analytics.session_metrics_daily s
