...
const defaultTranslate = (strings, ...values) =>
...
function getContextAndSettings() {
  ...
  const settings = managerConfig.settingsKey
    ? context.extensionSettings[managerConfig.settingsKey]
    : undefined;
...
export function updateCurrentPreset() {
  ...
  if (presetName === 'Default' && Object.keys(settings.presets).length > 1) {
    if (
      !confirm(
        managerConfig.t`Are you sure you want to update the Default preset? This will overwrite the original settings.`
      )
    ) {
      return;
    }
