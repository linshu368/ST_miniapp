...
const chargeVoiceUsage = vi.fn(async () => ({
...
vi.mock('../../infrastructure/repositories/MiniappWalletRepository.js', () => ({
  MiniappWalletRepository: class {
    chargeVoiceUsage = chargeVoiceUsage;
  },
}));
...
// 写稿分支本测试用不到，但 generate.ts 顶层 import 它，给个空实现即可
vi.mock('./voice-draft.js', () => ({
  draftSpokenText: vi.fn(),
}));
...
describe('runVoiceGeneration — 最终长度闸', () => {
  it('301 字不调 synthesizeSpeech、不扣费，markFailed(voice_text_too_long)', async () => {
    ...
    expect(chargeVoiceUsage).not.toHaveBeenCalled();
  ...
  it('刚好 300 字是合法的，走完合成 + 扣费 + markReady', async () => {
    ...
    expect(chargeVoiceUsage).toHaveBeenCalledTimes(1);
    expect(markReady).toHaveBeenCalledTimes(1);
    expect(markReady).toHaveBeenCalledWith(
      'a2',
      expect.objectContaining({ creditsCharged: 15, chargeId: 'a2' })
    );
...
describe('runVoiceGeneration — Q4 扣费时余额不足', () => {
  it('TTS 已成功但扣费返回 insufficient_balance：音频仍 markReady，credits_charged=0', async () => {
    chargeVoiceUsage.mockResolvedValueOnce({
      wallet: { user_id: 'u1', main_credits: 0, bonus_credits: 0, total_credits: 0 },
  ...
  it('扣费幂等命中 already_charged：按已扣记账，不重复扣', async () => {
    ...
    expect(markReady).toHaveBeenCalledWith(
      'a5',
      expect.objectContaining({ creditsCharged: 15, chargeId: 'a5' })
    );
  });
});
