...
import { chat, chat_metadata } from '../runtime/host.js';
...
function getFirstDisplayedMessageId() {
  // Backend port: ST reads `document.querySelector('#chat .mes')` to find
  // the first rendered message. There is no DOM here, so we always return
  // null (rendered as empty string by the macro handler). Step 0 baseline
  // cases never assert on this macro's output.
  return null;
}
