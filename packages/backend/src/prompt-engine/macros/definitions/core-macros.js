import { seedrandom, droll } from '../runtime/lib.js';
import {
  chat_metadata,
  main_api,
  getMaxPromptTokens,
...
import { getStringHash, isFalseBoolean } from '../runtime/utils.js';
import { inject_ids } from '../runtime/constants.js';
...
export function registerCoreMacros() {
  ...
  MacroRegistry.registerMacro('input', {
    ...
    // Backend port: ST reads `document.querySelector('#send_textarea').value`.
    // We have no DOM, so this resolves to whatever the caller passed via
    // `ctx.userInput` (defaults to '').
