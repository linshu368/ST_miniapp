...
import { isMobile } from '../runtime/host.js';
// Backend port (Step 0):
//   - parseMesExamples / formatInstructModeExamples are deliberately NOT
//     imported. The {{mesExamples}} handler is short-circuited to return
//     env.character.mesExamplesRaw (same as {{mesExamplesRaw}}). Real
//     instruct-mode formatting will be wired back in Step 1.
//   - main_api / power_user are no longer needed here because of the
//     above short-circuit.
...
export function registerEnvMacros() {
  ...
  MacroRegistry.registerMacro('mesExamples', {
    ...
    // Step 0 short-circuit: returns the same raw value as {{mesExamplesRaw}}.
    // The real ST handler runs parseMesExamples → (optionally) formatInstructModeExamples.
    // We defer that to Step 1 (Instruct format) so we don't drag instruct-mode.js
    // into Step 0. None of the 9 baseline cases use {{mesExamples}}, so this
    // short-circuit is invisible to byte-exact diffs.
    handler: ({ env }) => env.character.mesExamplesRaw ?? '',
  });
