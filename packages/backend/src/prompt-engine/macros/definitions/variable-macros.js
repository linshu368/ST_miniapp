...
import { variables } from '../runtime/variables.js';

/**
 * Registers variable-related {{...}} macros that operate on local and global
 * variables (e.g. {{setvar}}, {{getvar}}, {{incvar}}, etc.).
 *
 * Backend port: ST reads `SillyTavern.getContext().variables` at the time
 * registerVariableMacros() runs. Here we replace it with a module-level
 * `variables` namespace from `runtime/variables.js`, which has the same
 * { local, global }.{ get, set, del, add, inc, dec, has } shape and is
 * backed by `chat_metadata.variables` (local) and a caller-injected store
 * (global) — see runtime/variables.js for details.
 */
