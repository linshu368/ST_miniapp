...
// Backend port: ST's MacroDiagnostics has a UI onboarding popup that
// guides users to enable the experimental macro engine. On the backend
// we always run with the new engine, headless, no DOM. The original
// imports (`t`, `Popup`, `power_user`, `accountStorage`, `SimpleMutex`)
// are dropped along with the onboarding function below — every
// diagnostic / error helper that the engine actually invokes is
// preserved 1:1.
...
/**
 * Headless no-op replacement for ST's UI onboarding popup. Kept as an
 * exported async function so any future caller wiring in Step 1+ won't
 * crash; on the backend the experimental macro engine is always on.
 *
 * @param {string|null} _feature
 * @returns {Promise<void>}
 */
export async function onboardingExperimentalMacroEngine(_feature = null) {
  /* no-op on backend */
}
