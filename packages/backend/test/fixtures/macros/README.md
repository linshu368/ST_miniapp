...
| ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `{{roll}}`                                                                                                                    | `seedrandom(input.options.seed, { global: true })` overrides `Math.random` for the duration of the case, then restores it. `droll` uses `Math.random` so `{{roll}}` becomes fully deterministic.                                                                                                                                                                                                                                                                                                                                       |
| `{{random}}`                                                                                                                  | The macro's `handler` is **replaced** for the duration of the case by `installDeterministicRandomShim` in `adapters/macros.js`. The shim seeds with `JSON.stringify([input.options.seed, globalOffset, list])`. This is required because ST's stock `{{random}}` calls `seedrandom('added entropy.', { entropy: true })` which reads from a closure-private entropy pool — `Math.random`/`crypto` mocks cannot fix it. miniAPP **MUST** apply the same shim under the same seed contract for the byte-exact baseline diff to be valid. |
