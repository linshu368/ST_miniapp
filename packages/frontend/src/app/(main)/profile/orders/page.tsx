  ...
  return (
    <main className="app-page mx-auto flex max-w-md flex-col">
      ...
      <div
        role="tablist"
        aria-label="订单筛选"
        className="flex gap-2 border-b border-border bg-background/60 px-4 py-3 backdrop-blur-md"
      >
        {TABS.map((t) => {
      ...
      <section className="flex flex-1 flex-col gap-3 px-4 py-4">
        ...
        {query.isFetchingNextPage ? (
          ...
          <div className="py-4 text-center text-xs text-muted-foreground font-medium tracking-widest">
            NO MORE
          </div>
        ) : null}
      </section>
      ...
      <Sheet open={openOrder !== null} onOpenChange={(open) => !open && setOpenOrder(null)}>
        <SheetContent
          side="bottom"
          className="max-w-md rounded-t-[24px] border-border bg-popover p-0 text-popover-foreground shadow-2xl"
        >
...
function OrderRow({ order, onOpen }: { order: PaymentOrder; onOpen: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="app-surface group flex items-center justify-between p-4 text-left transition-all hover:border-primary/30 hover:shadow-md"
    >
      <div className="flex items-center gap-4">
        <div
          className={cn(
            'flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-muted',
            tone
          )}
        >
        ...
        <div className="min-w-0">
          ...
          <div className="mt-0.5 text-[10px] text-muted-foreground font-medium uppercase tracking-wider">
...
function statusVisual(status: PaymentOrderStatus) {
  switch (status) {
    ...
    case 'pending':
      return { Icon: Clock, tone: 'text-primary' };
    case 'expired':
...
function OrderDetail({ order, onClose }: { order: PaymentOrder; onClose: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-5 p-6 pb-[calc(env(safe-area-inset-bottom)+1.5rem)] text-popover-foreground">
      <div aria-hidden className="mx-auto h-1 w-12 rounded-full bg-muted mb-2" />

      <div className="flex items-center justify-between">
        <SheetTitle className="text-lg font-bold text-popover-foreground">订单明细</SheetTitle>
        <span
          className={cn(
            'rounded-full px-3 py-1 text-[11px] font-bold border border-border',
            order.status === 'completed' && 'bg-emerald-500/10 text-emerald-400',
            order.status === 'pending' && 'bg-amber-500/10 text-primary',
      ...
      <Button
        variant="ghost"
        onClick={onClose}
        className="mt-4 h-12 w-full rounded-2xl bg-muted text-[15px] font-bold text-foreground hover:bg-muted border border-border"
      >
        关闭
      </Button>
    </div>
...
function CopyableText({
  ...
  return (
    <button
      type="button"
      onClick={onCopy}
      className="inline-flex items-center gap-1.5 break-all text-right font-mono text-[11px] tracking-wider text-muted-foreground hover:text-foreground/80 transition-colors"
    >
      <span>{value}</span>
