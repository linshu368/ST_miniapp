...
export default function TavernChatPage() {
  ...
  const [pendingOpenChat, setPendingOpenChat] = useState<{
    fileName: string;
    avatar: string;
  } | null>(null);
  ...
  useEffect(() => {
    if (!pendingOpenChat || bridgeStatus !== 'ready' || !characterId) return;
    let cancelled = false;
    platformAction('openChat', pendingOpenChat)
      .then((result) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        ...
        if (!cancelled) setEntryError('历史对话加载失败，请重试。');
      });
    return () => {
      cancelled = true;
    };
  }, [bridgeStatus, characterId, pendingOpenChat]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      const latestChatPromise = fetchLatestUserChat(characterId)
        .then((data) => {
          const item = data.item;
          if (!item || !isSafeChatFileName(item.fileName)) return null;
          // 列表入口携带的 chat 只作一致性提示；目标必须由后端确认，避免异常参数误跳。
          if (requestedChat && requestedChat !== item.fileName) {
            console.warn('[TavernChatPage] ignored stale or invalid requested chat');
          }
          return item;
        })
        .catch((err) => {
      ...
      platformAction('selectCharacter', {
        avatar,
        forceNewChat: false,
        skipChatLoad: Boolean(latestChat),
      })
        .then(async (result) => {
          ...
          if (!cancelled) {
            if (latestChat && latestChat.fileName !== result.chatId) {
              const target = {
                fileName: latestChat.fileName,
                avatar: latestChat.characterAvatar || avatar,
              };
              if (bridgeStatusRef.current !== 'ready') {
                setPendingOpenChat(target);
                return;
              }
              const opened = await platformAction('openChat', target);
              useSTMirrorStore.getState().updatePartial({ currentChatId: opened.chatId });
            }
            setReadyCharacterId(characterId);
