...
export function VipPlanCard({
  ...
  variant?: 'detail' | 'recharge';
  checkinBaseCredits?: number;
  checkinVipCredits?: number;
}) {
  const isRecharge = variant === 'recharge';
  const checkinTotal =
    typeof checkinBaseCredits === 'number' && typeof checkinVipCredits === 'number'
      ? checkinBaseCredits + checkinVipCredits
      : null;
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected
          ? 'border-primary/80 bg-gradient-to-br from-primary/10 to-card shadow-[inset_0_0_0_1px_rgba(215,174,100,0.18),0_0_18px_rgba(215,174,100,0.08)]'
          : 'border-border bg-card hover:border-primary/30 hover:bg-secondary'
      )}
    >
      <div className="flex items-center justify-between gap-3">
        <span className="text-[15px] font-bold text-foreground">
          {isRecharge ? `VIP ${plan.title}` : plan.title}
        </span>
      ...
      {isRecharge ? (
        <p className="mt-1 text-[11px] leading-relaxed text-foreground/75">
          ...
          {checkinTotal !== null && typeof checkinVipCredits === 'number'
            ? ` · 签到共 ${formatNumber(checkinTotal)}/天（额外 +${formatNumber(checkinVipCredits)}）`
            : ''}
          {' · 标准/旗舰畅用'}
          {discountLabel ? ` · 全部模型 ${discountLabel}` : ''}
        </p>
      ) : (
      ...
      {plan.bonus_credits > 0 ? (
        <span className="mt-2 inline-flex items-center gap-1 rounded-full border border-primary/30 bg-primary/5 px-2 py-0.5 text-[10px] font-semibold text-primary">
          ...
          {isRecharge ? '赠轻量专项' : '赠送专项'} {formatNumber(plan.bonus_credits)} 星尘
        </span>
      ) : plan.badge_text ? (
