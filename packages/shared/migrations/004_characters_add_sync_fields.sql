...
--   - 新方案设计文档第 95 行：是否上架（is_published）+ 是否可用（is_active）+ is_default + 排序
--
-- 字段语义：
--   - is_default：阶段一新用户初始化时的默认激活卡（写入 user_st_settings.settings_jsonb.active_character）
--   - is_published：是否上架（大厅与同步引擎只下发 is_published=true 且 is_active=true 的卡）
--   - is_active：是否可用（停用后老用户已物化卡不可继续使用）
