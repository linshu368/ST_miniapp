...
--
-- Production compatibility forward-fix for 20260921_vip_billing_schema.sql.
--
-- Why: Production already has the pg_cron extension for existing operations. The
-- historical migration incorrectly rejected cron.job even though VIP reminders use
-- Railway Cron, not pg_cron. That file is already ledger-applied in TEST and must
-- not be changed. This file recreates its schema contract without touching pg_cron.
--
-- This migration is a one-time Production substitute, not a general schema
-- reconciler. Production must have none of the four VIP tables; any existing or
-- partial VIP schema is unsafe to rewrite and stops for a separate forward-fix.
-- No migration writes cron.job or creates a pg_cron schedule.
--
-- 前置：
...
--   若 Production 的历史 20260921 文件未记账但 pg_cron 已存在，执行本文件替代它。
-- 执行：GitHub Actions Database Migration，一次一个文件；不得在 SQL Editor 手工执行。
-- 恢复：不卸载 pg_cron；失败在事务内回滚，已提交后用新的 forward-fix。
...
BEGIN
  ...
  SELECT count(*) INTO v_existing_vip_tables
  FROM (VALUES
    (to_regclass('billing.vip_memberships')),
    (to_regclass('billing.vip_purchase_grants')),
    (to_regclass('billing.feature_free_trials')),
    (to_regclass('billing.wallet_refunds'))
  ) AS vip_tables(object_id)
  WHERE object_id IS NOT NULL;
  IF v_existing_vip_tables <> 0 THEN
    RAISE EXCEPTION '20260924_vip_billing_schema_pg_cron_compat: existing VIP schema (%/4 tables); stop for a separate forward-fix', v_existing_vip_tables;
  END IF;
...
-- ── 2. VIP 会员投影 ────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS billing.vip_memberships (
...
CREATE INDEX IF NOT EXISTS idx_vip_memberships_valid_until
...
-- ── 3. 订单履约事实 ────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS billing.vip_purchase_grants (
...
CREATE INDEX IF NOT EXISTS idx_vip_purchase_grants_user_granted
...
-- ── 4. 媒体免费次数公共事实 ───────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS billing.feature_free_trials (
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_feature_free_trials_reference
  ON billing.feature_free_trials (reference_id);

CREATE UNIQUE INDEX IF NOT EXISTS uq_feature_free_trials_occupying_ordinal
  ON billing.feature_free_trials (user_id, feature, ordinal)
  WHERE status IN ('reserved', 'consumed');

CREATE INDEX IF NOT EXISTS idx_feature_free_trials_user_feature
  ON billing.feature_free_trials (user_id, feature, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_feature_free_trials_reserved_until
...
-- ── 5. 原路退款事实 ───────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS billing.wallet_refunds (
...
CREATE INDEX IF NOT EXISTS idx_wallet_refunds_user_created
...
ALTER TABLE billing.payment_orders
  ...
  DROP CONSTRAINT IF EXISTS payment_orders_product_snapshot_check,
...
ALTER TABLE miniapp_features.notifications
  ...
  DROP CONSTRAINT IF EXISTS notifications_vip_expiry_shape_check,
...
BEGIN
  ...
  -- Existing pg_cron is an independent Production scheduler. VIP reminder dispatch
  -- remains a Railway Cron consumer and this migration never creates cron jobs.
END;
$$;
