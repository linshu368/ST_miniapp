 ...
 *
 * 修复目标：模型输出中 <disclaimer> 包装标签可能直接暴露给用户。
 *   <think> / <thinking> 交给 ST 原生 reasoning 解析器展示为可折叠过程；
 *   不再用展示正则整块删除，避免闭合标签抵达时内容一帧消失。
 *
...
function injectSafetyNetScripts(): void {
  try {
    ...
    const obsoleteThinkingIndex = regex.findIndex(
      (script: Record<string, unknown>) => script.id === 'platform-safety-net-thinking'
    );
    if (obsoleteThinkingIndex >= 0) regex.splice(obsoleteThinkingIndex, 1);
