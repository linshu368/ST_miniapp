...
| public/script.js             | 模块顶层           | TEMP DEBUG 停摆定位探针（miniapp_fast_boot 门控）：fetch 包装记录请求生命周期 + 静默异常捕获，暴露 `__miniappFetchLog` / `__miniappBootErrors` 供父窗口收割 | boot 楔死请求定位（区分「发出未归」与「从未发出」）        | 随 [iframe-timing] 埋点整体移除                                  |
