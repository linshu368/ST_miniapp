...
} from './scripts/extensions.js?miniapp_v=20260713c';
import {
...
let exportPopper = Popper.createPopper(
  ...
  document.getElementById('export_format_popup'),
  {
    placement: 'left',
  }
);
...
async function firstLoadInit() {
  try {
    ...
    toastr.error(t`Couldn't get CSRF token. Please refresh the page.`, t`Error`, {
      timeOut: 0,
      extendedTimeOut: 0,
...
export async function getOneCharacter(avatarUrl) {
  ...
  if (response.ok) {
    ...
    if (indexOf !== -1) {
      ...
      toastr.error(t`Character ${avatarUrl} not found in the list`, t`Error`, {
        timeOut: 5000,
        preventDuplicates: true,
      });
    }
 ...
 * @returns {string} The string with substituted parameters.
 */
export function substituteParamsLegacy(
  content,
  _name1,
  _name2,
  _original,
  ...
  if (accountStorage.getItem('slash_command_experimental_engine_warning_shown') !== 'true') {
    ...
    if (/{{\s*if/.test(content)) feature = '{{if}} macro';
      ...
      feature = 'nested macro';
    else if (/{{(?:greeting|charFirstMessage)(?:::\d+)?}}/i.test(content))
      feature = 'greeting macro';
...
export function substituteParams(content, options = {}) {
  ...
  if (!power_user?.experimental_macro_engine) {
    return substituteParamsLegacy(
      content,
      options.name1Override,
      options.name2Override,
      options.original,
      options.groupOverride,
      options.replaceCharacterCard,
  ...
  const ctx =
    /** @type {import('./scripts/macros/engine/MacroEnvBuilder.js').MacroEnvRawContext} */ ({
      content,
  ...
  async generate() {
    ...
    try {
      ...
      for await (const { text, swipes, logprobs, toolCalls, state } of this.generator()) {
        ...
        await sw.tick(
          async () => await this.onProgressStreaming(this.messageId, this.continueMessage + text)
        );
      }
...
export async function Generate(
  ...
  if (selected_group && Array.isArray(groupDepthPrompts) && groupDepthPrompts.length > 0) {
    ...
    const depthPromptDepth =
      characters[this_chid]?.data?.extensions?.depth_prompt?.depth ?? depth_prompt_depth_default;
  ...
  async function getCombinedPrompt(isNegative) {
    ...
    const combine = () => {
      ...
      let combinedPrompt = [combinedStoryString, mesExmString, mesSendString, generatedPromptCache]
        .join('')
        .replace(/\r/gm, '');
  ...
  async function finishGenerating() {
    ...
    let additionalPromptStuff = {
      ...
      instruction:
        main_api !== 'openai' && power_user.sysprompt.enabled
          ? substituteParams(
              power_user.prefer_character_prompt && system ? system : power_user.sysprompt.content
            )
          : '',
      userPersona:
        power_user.persona_description_position == persona_description_positions.IN_PROMPT
    ...
    if (isStreamingEnabled() && type !== 'quiet') {
      ...
      const isStreamFinished =
        streamingProcessor && !streamingProcessor.isStopped && streamingProcessor.isFinished;
  ...
  function onError(exception) {
    ...
    if (typeof exception?.error?.message === 'string') {
      toastr.error(exception.error.message, t`Text generation error`, {
        timeOut: 10000,
...
function formatMessageHistoryItem(chatItem, isInstruct, forceOutputSequence) {
  ...
  let textResult =
    chatItem?.name && shouldPrependName ? `${itemName}: ${chatItem.mes}\n` : `${chatItem.mes}\n`;
...
function parseTokenCounts(counts, thisPromptBits) {
  ...
  thisPromptBits.push({
    ...
    oaiPromptTokens:
      getSum(counts?.prompt, counts?.charDescription, counts?.charPersonality, counts?.scenario) ||
      0,
    ...
    case 'textgenerationwebui':
      switch (textgen_settings.type) {
        case textgen_types.LLAMACPP:
          {
            logprobs =
              data?.completion_probabilities?.map((x) => parseTextgenLogprobs(x.content, [x])) ||
              null;
          }
          break;
...
export async function saveReply({
  ...
  if (
    ...
    type != 'appendFinal' &&
    chat.length &&
    (lastMessage.swipe_id === undefined || lastMessage.is_user)
  ) {
    type = 'normal';
  }
...
async function read_avatar_load(input) {
  if (input.files && input.files[0]) {
    ...
    if (!power_user.never_resize_avatars) {
      const dlg = new Popup('Set the crop position of the avatar image', POPUP_TYPE.CROP, '', {
        cropImage: fileData,
      });
...
export function callPopup(
  ...
  $dialoguePopup
    .toggleClass('wide_dialogue_popup', !!wide)
...
export async function updateSwipeCounter(
  mesId,
  { message = undefined, messageElement = undefined } = {}
) {
  message ??= chat[mesId];
...
export async function swipe(
  ...
  const mesId = Number(
    forceMesId ??
      event?.currentTarget?.closest('.mes')?.getAttribute('mesid') ??
      messageIndex ??
      chat.length - 1
  );
  ...
  function expandNewMessage(thisMesDiv) {
    ...
    const is_animation_scroll =
      chatElement.scrollTop() >= chatElement.prop('scrollHeight') - chatElement.outerHeight() - 10;
...
export async function swipe_left(event, { source, repeated, message } = {}) {
  await swipe.call(this, event, SWIPE_DIRECTION.LEFT, {
    source: source,
    repeated: repeated,
    message: message,
  });
...
export async function swipe_right(event = null, { source, repeated, message } = {}) {
  await swipe.call(this, event, SWIPE_DIRECTION.RIGHT, {
    source: source,
    repeated: repeated,
    message: message,
  });
...
export async function processDroppedFiles(files, data = new Map()) {
  ...
  const allowedExtensions = ['charx', 'byaf'];
    ...
    if (item.type === 'text/uri-list') {
      ...
      try {
        for (const uri of uris) {
          ...
          const fileName =
            request.headers.get('Content-Disposition')?.split('filename=')[1]?.replace(/"/g, '') ||
            uri.split('/').pop() ||
            'file.png';
    ...
    () => {
      ...
      toastr.info(
        'Regenerate warning is now ' +
          (accountStorage.getItem('RegenerateWithCtrlEnter') === 'true' ? 'disabled' : 'enabled')
      );
    }
  );
...
jQuery(async function () {
  ...
  $(document).on('click', '.mes .avatar', function () {
    ...
    if ($(`.zoomed_avatar[forChar="${charname}"]`).length) {
      ...
      if (
        messageElement.attr('is_user') == 'true' ||
        (messageElement.attr('is_system') == 'true' && !isValidCharacter)
      ) {
  ...
  $(document).on('keydown', function (e) {
    if (e.key === 'Escape' && !e.originalEvent.isComposing) {
      const isEditVisible =
        $('#curEditTextarea').is(':visible') || $('.reasoning_edit_textarea').length > 0;
