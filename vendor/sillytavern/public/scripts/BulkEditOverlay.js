  ...
  static favorite = async (characterId) => {
    ...
    if (!mergeResponse.ok) {
      mergeResponse
        .json()
        .then((json) =>
          toastr.error(`Character not saved. Error: ${json.message}. Field: ${json.error}`)
        );
    }
...
class BulkEditOverlay {
  ...
  set state(newState) {
    ...
    eventSource.emit(event_types.CHARACTER_GROUP_OVERLAY_STATE_CHANGE_BEFORE, newState).then(() => {
      this.#state = newState;
  ...
  clearSelectedCharacters = () => {
    document
      .querySelectorAll('#' + BulkEditOverlay.containerId + ' .' + BulkEditOverlay.selectedClass)
