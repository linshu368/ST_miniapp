...
function setNotePositionCommand(_, text) {
  ...
  if (text) {
    ...
    $(`input[name="extension_floating_position"][value="${position}"]`)
      .prop('checked', true)
      .trigger('input');
    toastr.info(t`Author's Note position updated`);
  }
  return Object.keys(validPositions).find(
    (key) => validPositions[key] == chat_metadata[metadata_keys.position]
  );
}
...
function setNoteRoleCommand(_, text) {
  ...
  return Object.keys(validRoles).find(
    (key) => validRoles[key] == chat_metadata[metadata_keys.role]
  );
}
...
const setMainPromptTokenCounterDebounced = debounce(
  async (value) =>
    $('#extension_floating_prompt_token_counter').text(await getTokenCountAsync(value)),
  debounce_timeout.relaxed
);
const setCharaPromptTokenCounterDebounced = debounce(
  async (value) =>
    $('#extension_floating_chara_token_counter').text(await getTokenCountAsync(value)),
  debounce_timeout.relaxed
);
const setDefaultPromptTokenCounterDebounced = debounce(
  async (value) =>
    $('#extension_floating_default_token_counter').text(await getTokenCountAsync(value)),
  debounce_timeout.relaxed
);
...
function loadSettings() {
  ...
  chat_metadata[metadata_keys.prompt] =
    chat_metadata[metadata_keys.prompt] ?? extension_settings.note.default ?? '';
  chat_metadata[metadata_keys.interval] =
    chat_metadata[metadata_keys.interval] ??
    extension_settings.note.defaultInterval ??
    DEFAULT_INTERVAL;
  ...
  chat_metadata[metadata_keys.depth] =
    chat_metadata[metadata_keys.depth] ?? extension_settings.note.defaultDepth ?? DEFAULT_DEPTH;
  chat_metadata[metadata_keys.role] =
    chat_metadata[metadata_keys.role] ?? extension_settings.note.defaultRole ?? DEFAULT_ROLE;
  ...
  $('#extension_floating_allow_wi_scan').prop(
    'checked',
  ...
  $(
    `input[name="extension_floating_position"][value="${chat_metadata[metadata_keys.position]}"]`
  ).prop('checked', true);
  ...
  if (extension_settings.note.chara && getContext().characterId !== undefined) {
    ...
    $(
      `input[name="extension_floating_char_position"][value="${charaNote?.position ?? chara_note_position.replace}"]`
    ).prop('checked', true);
  } else {
    $('#extension_floating_chara').val('');
    $('#extension_use_floating_chara').prop('checked', false);
    $(
      `input[name="extension_floating_char_position"][value="${chara_note_position.replace}"]`
    ).prop('checked', true);
  }
  ...
  $(
    `input[name="extension_default_position"][value="${extension_settings.note.defaultPosition}"]`
  ).prop('checked', true);
}
...
export function setFloatingPrompt() {
  ...
  let lastMessageNumber =
    Array.isArray(context.chat) && context.chat.length
      ? context.chat.filter((m) => m.is_user).length
      : 0;
...
function onANMenuItemClick() {
  ...
  if ($ANcontainer.css('display') !== 'flex') {
    ...
    if ($('#ANBlockToggle').siblings('.inline-drawer-content').css('display') !== 'block') {
      $ANcontainer.addClass('resizing');
...
async function onChatChanged() {
  ...
  const tokenCounter1 = chat_metadata[metadata_keys.prompt]
    ? await getTokenCountAsync(chat_metadata[metadata_keys.prompt])
    : 0;
  ...
  const tokenCounter3 = extension_settings.note.default
    ? await getTokenCountAsync(extension_settings.note.default)
    : 0;
...
export function initAuthorsNote() {
  ...
  $('input[name="extension_floating_char_position"]').on(
    'change',
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      unnamedArgumentList: [new SlashCommandArgument('text', [ARGUMENT_TYPE.STRING], false)],
      helpString: `
            <div>
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      unnamedArgumentList: [new SlashCommandArgument('number', [ARGUMENT_TYPE.NUMBER], false)],
      helpString: `
            <div>
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      namedArgumentList: [],
      unnamedArgumentList: [
        new SlashCommandArgument('position', [ARGUMENT_TYPE.STRING], false, false, null, [
          'before',
          'after',
          'chat',
        ]),
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      namedArgumentList: [],
      unnamedArgumentList: [
        new SlashCommandArgument('role', [ARGUMENT_TYPE.STRING], false, false, null, [
          'system',
          'user',
          'assistant',
        ]),
