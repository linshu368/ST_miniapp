...
function onLockBackgroundClick(event = null) {
  ...
  const urlToLock = event
    ? $(event.target).closest('.bg_example').data('url')
...
function syncGroupSelectionUi() {
  ...
  const showRemoveFromCurrentFolderButton =
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (folder) {
        ...
        if (coverUrl) {
          $(`.bg_folder_tile[data-folder-id="${folder.id}"] .bg_folder_tile_cover`).css(
            'background-image',
            `url('${coverUrl}')`
          );
        }
...
async function convertFileIfVideo(formData) {
  ...
  try {
    toastMessage = toastr.info(
      t`Preparing video for upload. This may take several minutes.`,
      t`Please wait`,
      { timeOut: 0, extendedTimeOut: 0 }
    );
    const sourceBuffer = await file.arrayBuffer();
    const convertedBuffer = await globalThis.convertVideoToAnimatedWebp({
      buffer: new Uint8Array(sourceBuffer),
      name: file.name,
    });
    const convertedFileName = file.name.replace(/\.[^/.]+$/, '.webp');
    const convertedFile = new File([new Uint8Array(convertedBuffer)], convertedFileName, {
      type: 'image/webp',
    });
...
const debouncedOnBackgroundFilterInput = debounce(
...
export function initBackgrounds() {
  ...
  $(document)
    .off('click', '.bg_example')
    .on('click', '.bg_example', onSelectBackgroundClick)
    .off('click', '.bg_example .mobile-only-menu-toggle')
    .on('click', '.bg_example .mobile-only-menu-toggle', function (e) {
      e.stopPropagation();
    ...
    .off('blur', '.bg_example.mobile-menu-open')
    .on('blur', '.bg_example.mobile-menu-open', function () {
      if (!$(this).is(':focus-within')) {
        $(this).removeClass('mobile-menu-open');
      }
    })
    .off('click', '.jg-button')
    .on('click', '.jg-button', function (e) {
      e.stopPropagation();
  ...
  $('#bg_selection_mode_button').on('click', () =>
