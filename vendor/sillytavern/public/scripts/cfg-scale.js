...
function onCfgMenuItemClick() {
  ...
  if ($('#cfgConfig').css('display') !== 'flex') {
    ...
    if ($('#CFGBlockToggle').siblings('.inline-drawer-content').css('display') !== 'block') {
...
function loadSettings() {
  ...
  $('#chat_cfg_guidance_scale_counter').val(
  ...
  if (!selected_group) {
    ...
    $('#chara_cfg_guidance_scale_counter').val(
...
async function initialLoadSettings() {
  ...
  $('#global_cfg_guidance_scale_counter').val(
    extension_settings.cfg.global.guidance_scale.toFixed(2)
  );
...
export function initCfg() {
  ...
  $('input[name="cfg_prompt_combine"]').on('input', function () {
    const values =
      $('#cfgConfig')
        .find('input[name="cfg_prompt_combine"]')
        .filter(':checked')
        .map(function () {
...
export function getGuidanceScale() {
  ...
  const charaCfg = extension_settings.cfg.chara?.find(
    (e) => e.name === getCharaFilename(this_chid)
  );
...
export function getCfgPrompt(guidanceScale, isNegative, quiet = false) {
  ...
  if (guidanceScale.type === cfgType.global || cfgPromptCombine.includes(cfgType.global)) {
    splitCfgPrompt.unshift(
      substituteParams(
        isNegative
          ? extension_settings.cfg.global.negative_prompt
          : extension_settings.cfg.global.positive_prompt
      )
    );
  }
