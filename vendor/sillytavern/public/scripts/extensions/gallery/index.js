...
const isVideo = (/** @type {string} */ url) =>
  VIDEO_EXTENSIONS.some((ext) => new RegExp(`.${ext}$`, 'i').test(url));
...
async function initGallery(items, url) {
  ...
  const galleryMaxRows = clamp(
    Math.floor((window.innerHeight * 0.9 - 75) / thumbnailHeight),
    1,
    10
  );
...
function viewWithDragbox(items) {
  if (items && items.length > 0) {
    ...
    if (deleteModeActive) {
      Popup.show
        .confirm(t`Are you sure you want to delete this image?`, url)
...
async function listGalleryCommand(args) {
  try {
    let url =
      args.char ??
      (args.group ? groups.find((it) => it.name == args.group)?.id : null) ??
...
function addGalleryWandButton() {
  const showGalleryContainer =
    document.getElementById('gallery_wand_container') || document.getElementById('extensionsMenu');
