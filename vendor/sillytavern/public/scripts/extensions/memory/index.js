...
function loadSettings() {
  ...
  $(`input[name="memory_prompt_builder"][value="${extension_settings.memory.prompt_builder}"]`)
    .prop('checked', true)
    .trigger('input');
  $('#memory_override_response_length')
    .val(extension_settings.memory.overrideResponseLength)
    .trigger('input');
  $('#memory_max_messages_per_request')
    .val(extension_settings.memory.maxMessagesPerRequest)
    .trigger('input');
...
async function onPromptForceWordsAutoClick() {
  ...
  const promptAllowanceWords =
    maxPromptLengthWords - extension_settings.memory.promptWords - summaryPromptWords;
  ...
  const targetMessagesInPrompt =
    maxMessagesPerSummary > 0 ? maxMessagesPerSummary : Math.max(0, averageMessagesPerPrompt);
  const targetSummaryWords =
    targetMessagesInPrompt * averageMessageWordCount + promptAllowanceWords / 4;
  ...
  const ROUNDING = 100;
  extension_settings.memory.promptForceWords = Math.max(
    1,
...
async function onPromptIntervalAutoClick() {
  ...
  const targetMessagesInPrompt =
    maxMessagesPerSummary > 0 ? maxMessagesPerSummary : Math.max(0, averageMessagesPerPrompt);
  const adjustedAverageMessagesPerPrompt =
    targetMessagesInPrompt + (averageMessagesPerPrompt - targetMessagesInPrompt) / 4;
  ...
  const ROUNDING = 5;
  extension_settings.memory.promptInterval = Math.max(
    1,
...
function switchSourceControls(value) {
  $(
    '#summaryExtensionDrawerContents [data-summary-source], #memory_settings [data-summary-source]'
  ).each((_, element) => {
...
function onOverrideResponseLengthInput() {
  ...
  $('#memory_override_response_length_value').text(
...
async function onChatEvent() {
  // Module not enabled
  if (
    extension_settings.memory.source === summary_sources.extras &&
...
async function forceSummarizeChat(quiet) {
  ...
  const toast = quiet
    ? jQuery()
    : toastr.info('Summarizing chat...', 'Please wait', { timeOut: 0, extendedTimeOut: 0 });
  const value =
...
async function summarizeCallback(args, text) {
  ...
  const prompt = substituteParamsExtended(args.prompt || extension_settings.memory.prompt, {
    words: extension_settings.memory.promptWords,
  });
  ...
  try {
    switch (source) {
      ...
      case summary_sources.main:
        return removeReasoningFromString(
          await generateRaw({
            prompt: text,
            systemPrompt: prompt,
      ...
      case summary_sources.webllm: {
        ...
        const params =
          extension_settings.memory.overrideResponseLength > 0
...
async function getSummaryPromptForNow(context, force) {
  ...
  if (!conditionSatisfied && !force) {
    console.debug(
      `Summary conditions not satisfied (messages: ${messagesSinceLastSummary}, interval: ${extension_settings.memory.promptInterval}, words: ${wordsSinceLastSummary}, force words: ${extension_settings.memory.promptForceWords})`
    );
    return '';
  }
...
async function summarizeChatMain(context, force, skipWIAN) {
  ...
  if (
    [prompt_builders.RAW_BLOCKING, prompt_builders.RAW_NON_BLOCKING].includes(
      extension_settings.memory.prompt_builder
    )
  ) {
...
async function getRawSummaryPrompt(context, prompt) {
  ...
  for (let index = latestSummaryIndex + 1; index < chat.length; index++) {
    ...
    if (
      extension_settings.memory.maxMessagesPerRequest > 0 &&
...
function doPopout(e) {
  ...
  if ($('#summaryExtensionPopout').length === 0) {
    ...
    const originalHTMLClone = $(target)
      .parent()
      .parent()
      .parent()
    ...
    newElement
      .attr('id', 'summaryExtensionPopout')
      .css('opacity', 0)
    ...
    originalElement.html(
      '<div class="flex-container alignitemscenter justifyCenter wide100p"><small>Currently popped out</small></div>'
    );
    ...
    $('#summaryExtensionPopout').fadeOut(animation_duration, () => {
      $('#summaryExtensionPopoutClose').trigger('click');
    });
  }
}
...
function setupListeners() {
  ...
  $('#memory_force_summarize')
    .off('click')
    .on('click', () => forceSummarizeChat(false));
...
export async function init() {
  async function addExtensionControls() {
    const settingsHtml = await renderExtensionTemplateAsync('memory', 'settings', {
      defaultSettings,
    });
    ...
    SlashCommand.fromProps({
      ...
      namedArgumentList: [
        new SlashCommandNamedArgument(
          'source',
          'API to use for summarization',
          [ARGUMENT_TYPE.STRING],
          false,
          false,
          '',
          Object.values(summary_sources)
        ),
      ...
      helpString:
        'Summarizes the given text. If no text is provided, the current chat will be summarized. Can specify the source and the prompt to use.',
