...
export class QuickReplySet {
  ...
  async executeWithOptions(qr, options = {}) {
    ...
    if (input[0] == '/' && !this.disableSend) {
      ...
      if (options.isAutoExecute || options.isRun) {
        ...
        result = await executeSlashCommandsOnChatInput(
          input,
          Object.assign(execOptions, {
            scope: options.scope,
            source: `${this.name}.${qr.label}`,
  ...
  addQuickReply(data = {}) {
    const id =
      Math.max(
        this.idIndex,
        this.qrList.reduce((max, qr) => Math.max(max, qr.id), 0)
      ) + 1;
    ...
    qr.onTransfer = async () => {
      ...
      const dom = document.createElement('div');
      {
        ...
        const hintP = document.createElement('p');
        {
          ...
          {
            hint.textContent =
              'Type or arrows to select QR Set. Enter to transfer. Shift+Enter to copy.';
            hintP.append(hint);
          }
          dom.append(hintP);
        }
