...
export class SlashCommandHandler {
  ...
  init() {
    ...
    const localEnumProviders = {
      ...
      qrSets: (executor) =>
        QuickReplySet.list
          .filter(
            (qrSet) =>
              qrSet.name != String(executor.namedArgumentList.find((x) => x.name == 'set')?.value)
          )
      ...
      qrEntries: (executor) =>
        QuickReplySet.get(
          String(executor.namedArgumentList.find((x) => x.name == 'set')?.value)
        )?.qrList.map((qr) => {
          const icons = getExecutionIcons(qr);
          const message =
            `${qr.automationId ? `[${qr.automationId}]` : ''}${icons ? `[auto: ${icons}]` : ''} ${qr.title || qr.message}`.trim();
      ...
      qrExecutables: () => {
        ...
        const globalQrs = globalSetList
          .map((link) => link.set.qrList.map((qr) => ({ set: link.set, qr })))
          .flat();
        const chatQrs =
          chatSetList?.map((link) => link.set.qrList.map((qr) => ({ set: link.set, qr }))).flat() ??
          [];
        const otherQrs = QuickReplySet.list
          .filter(
            (set) =>
              !globalSetList.some(
                (link) =>
                  link.set.name === set.name &&
                  !chatSetList?.some((link) => link.set.name === set.name)
              )
          )
          .map((set) => set.qrList.map((qr) => ({ set, qr })))
          .flat();
          ...
          ...globalQrs.map(
            (x) =>
              new SlashCommandEnumValue(
                `${x.set.name}.${x.qr.label}`,
                `[global] ${x.qr.title || x.qr.message}`,
                enumTypes.name,
                enumIcons.qr
              )
          ),
    ...
    SlashCommandParser.addCommandObject(
      SlashCommand.fromProps({
        ...
        unnamedArgumentList: [new SlashCommandArgument('number', [ARGUMENT_TYPE.NUMBER], true)],
    ...
    SlashCommandParser.addCommandObject(
      SlashCommand.fromProps({
        ...
        callback: () => {
          toastr.warning(
            'The command /qrset has been deprecated. Use /qr-set, /qr-set-on, and /qr-set-off instead.'
          );
          return '';
        },
        helpString:
          '<strong>DEPRECATED</strong> – The command /qrset has been deprecated. Use /qr-set, /qr-set-on, and /qr-set-off instead.',
      })
    );
    SlashCommandParser.addCommandObject(
      SlashCommand.fromProps({
    ...
    SlashCommandParser.addCommandObject(
      SlashCommand.fromProps({
        ...
        namedArgumentList: [
          new SlashCommandNamedArgument(
            'visible',
            'whether the QR set should be visible',
            [ARGUMENT_TYPE.BOOLEAN],
            false,
            false,
            'true'
    ...
    SlashCommandParser.addCommandObject(
      SlashCommand.fromProps({
        ...
        namedArgumentList: [],
        unnamedArgumentList: [
          new SlashCommandArgument('set type', [ARGUMENT_TYPE.STRING], false, false, 'all', [
            'all',
            'global',
            'chat',
          ]),
    ...
    const qrArgs = [
      ...
      SlashCommandNamedArgument.fromProps({
        name: 'showlabel',
        description:
          'whether to show the label even when an icon is assigned, e.g., icon=fa-pencil showlabel=true',
        typeList: [ARGUMENT_TYPE.BOOLEAN],
        isRequired: false,
      }),
      new SlashCommandNamedArgument(
        'hidden',
        'whether the button should be hidden, e.g., hidden=true',
        [ARGUMENT_TYPE.BOOLEAN],
        false,
        false,
        'false'
      ...
      new SlashCommandNamedArgument(
        'startup',
        'auto execute on app startup, e.g., startup=true',
        [ARGUMENT_TYPE.BOOLEAN],
        false,
        false,
        'false'
      ...
      new SlashCommandNamedArgument(
        'user',
        'auto execute on user message, e.g., user=true',
        [ARGUMENT_TYPE.BOOLEAN],
        false,
        false,
        'false'
      ...
      new SlashCommandNamedArgument(
        'bot',
        'auto execute on AI message, e.g., bot=true',
        [ARGUMENT_TYPE.BOOLEAN],
        false,
        false,
        'false'
      ...
      new SlashCommandNamedArgument(
        'load',
        'auto execute on chat load, e.g., load=true',
        [ARGUMENT_TYPE.BOOLEAN],
        false,
        false,
        'false'
      ...
      new SlashCommandNamedArgument(
        'new',
        'auto execute on new chat, e.g., new=true',
        [ARGUMENT_TYPE.BOOLEAN],
        false,
        false,
        'false'
      ...
      new SlashCommandNamedArgument(
        'group',
        'auto execute on group member selection, e.g., group=true',
        [ARGUMENT_TYPE.BOOLEAN],
        false,
        false,
        'false'
      ...
      new SlashCommandNamedArgument(
        'generation',
        'auto execute before message generation, e.g., generation=true',
        [ARGUMENT_TYPE.BOOLEAN],
        false,
        false,
        'false'
      ...
      new SlashCommandNamedArgument(
        'title',
        'title / tooltip to be shown on button, e.g., title="My Fancy Button"',
    ...
    SlashCommandParser.addCommandObject(
      SlashCommand.fromProps({
        ...
        unnamedArgumentList: [new SlashCommandArgument('command', [ARGUMENT_TYPE.STRING], true)],
        helpString: `
    ...
    SlashCommandParser.addCommandObject(
      SlashCommand.fromProps({
        ...
        unnamedArgumentList: [new SlashCommandArgument('command', [ARGUMENT_TYPE.STRING])],
        helpString: `
                <div>
                    Updates Quick Reply.
                </div>
    ...
    SlashCommandParser.addCommandObject(
      SlashCommand.fromProps({
        ...
        helpString:
          'Deletes a Quick Reply from the specified set. (Label must be provided via named or unnamed argument)',
      })
    );
    SlashCommandParser.addCommandObject(
      SlashCommand.fromProps({
    ...
    const presetArgs = [
      new SlashCommandNamedArgument(
        'nosend',
        'disable send / insert in user input (invalid for slash commands)',
        [ARGUMENT_TYPE.BOOLEAN],
        false
      ),
      new SlashCommandNamedArgument(
        'before',
        'place QR before user input',
        [ARGUMENT_TYPE.BOOLEAN],
        false
      ),
      new SlashCommandNamedArgument(
        'inject',
        'inject user input automatically (if disabled use {{input}})',
        [ARGUMENT_TYPE.BOOLEAN],
        false
      ),
    ];
      ...
      SlashCommand.fromProps({
        ...
        callback: (args, value) => {
          ...
          if (qr) {
            ...
            const closure = parser.parse(
              qr.message,
              true,
              [],
              args._abortController,
              args._debugController
            );
            ...
            const candidates = closure.executorList
              ...
              .map((executor) => ({
                key:
                  executor.namedArgumentList.find((arg) => arg.name == 'key')?.value ??
                  executor.unnamedArgumentList[0].value,
                value:
                  executor.unnamedArgumentList[
                    executor.namedArgumentList.find((arg) => arg.name == 'key') ? 0 : 1
                  ].value,
              }));
            for (let i = 0; i < value.length; i++) {
              ...
              if (!pick)
                throw new Error(`No scoped closure named "${srcName}" found in "${args.from}"`);
  ...
  getQrByLabel(setName, label) {
    ...
    if (!qr) {
      toastr.error(
        `No Quick Reply with the label "${label}" could be found in the set "${setName}"`
      );
    }
    return qr;
  }
  ...
  createQuickReply(args, message) {
    try {
      this.api.createQuickReply(args.set ?? '', args.label ?? '', {
        icon: args.icon,
  ...
  getQuickReply(args) {
    ...
    try {
      return JSON.stringify(
        this.api.getQrByLabel(args.set, args.id !== undefined ? Number(args.id) : args.label)
      );
    } catch (ex) {
      toastr.error(ex.message);
    }
      ...
      this.api.updateQuickReply(
        ...
        args.id !== undefined ? Number(args.id) : (args.label ?? ''),
        {
          ...
          executeOnGroupMemberDraft:
            args.group === undefined ? undefined : isTrueBoolean(args.group),
          executeOnNewChat: args.new === undefined ? undefined : isTrueBoolean(args.new),
          executeBeforeGeneration:
            args.generation === undefined ? undefined : isTrueBoolean(args.generation),
          automationId: args.automationId ?? '',
        }
      );
    } catch (ex) {
  ...
  deleteQuickReply(args, label) {
    try {
      this.api.deleteQuickReply(
        args.set,
        args.id !== undefined ? Number(args.id) : (args.label ?? label)
      );
    } catch (ex) {
      toastr.error(ex.message);
    }
  ...
  async createSet(name, args) {
    try {
      await this.api.createSet(args.name ?? name ?? '', {
  ...
  async updateSet(name, args) {
    try {
      await this.api.updateSet(args.name ?? name ?? '', {
