...
export class SettingsUi {
  ...
  rerender() {
    ...
    // @ts-ignore
    Array.from(
      this.template.querySelector('.inline-drawer-content').cloneNode(true).children
    ).forEach((el) => {
      content.append(el);
    });
    this.prepareDom();
  ...
  async render() {
    if (!this.dom) {
      const response = await fetch('/scripts/extensions/quick-reply/html/settings.html', {
        cache: 'no-store',
      });
      if (response.ok) {
  ...
  prepareQrEditor() {
    // qr editor
    this.dom
      .querySelector('#qr--set-rename')
      .addEventListener('click', async () => this.renameQrSet());
    ...
    this.dom
      .querySelector('#qr--set-export')
      .addEventListener('click', async () => this.exportQrSet());
    this.dom
      .querySelector('#qr--set-duplicate')
      .addEventListener('click', async () => this.duplicateQrSet());
    this.dom
      .querySelector('#qr--set-delete')
      .addEventListener('click', async () => this.deleteQrSet());
  ...
  async onQrListSort() {
    this.currentQrSet.qrList = Array.from(this.qrList.querySelectorAll('.qr--set-item')).map(
      (it, idx) => {
        const qr = this.currentQrSet.qrList.find(
          (qr) => qr.id == Number(it.getAttribute('data-id'))
        );
        it.setAttribute('data-order', String(idx));
        return qr;
      }
  ...
  async deleteQrSet() {
    const confirmed = await Popup.show.confirm(
      'Delete Quick Reply Set',
      `Are you sure you want to delete the Quick Reply Set "${this.currentQrSet.name}"?<br>This cannot be undone.`
    );
    if (confirmed) {
  ...
  async renameQrSet() {
    const newName = await Popup.show.input(
      'Rename Quick Reply Set',
      'Enter a new name:',
      this.currentQrSet.name
    );
  ...
  async addQrSet() {
    const name = await Popup.show.input(
      ...
      'Enter a name for the new Quick Reply Set:'
    );
    if (name && name.length > 0) {
      const oldQrs = QuickReplySet.get(name);
      if (oldQrs) {
        const replace = Popup.show.confirm(
          'Replace existing World Info',
          `A Quick Reply Set named "${name}" already exists.<br>Do you want to overwrite the existing Quick Reply Set?<br>The existing set will be deleted. This cannot be undone.`
        );
        if (replace) {
        ...
        qrs.name = name;
        qrs.addQuickReply();
        const idx = QuickReplySet.list.findIndex(
          (it) => it.name.toLowerCase().localeCompare(name.toLowerCase()) == 1
        );
        if (idx > -1) {
          QuickReplySet.list.splice(idx, 0, qrs);
        } else {
    ...
    try {
      ...
      if (!Number.isInteger(props.version) || typeof props.name != 'string') {
        ...
        if (oldQrs) {
          ...
          const idx = QuickReplySet.list.findIndex(
            (it) => it.name.toLowerCase().localeCompare(qrs.name.toLowerCase()) == 1
          );
          if (idx > -1) {
            QuickReplySet.list.splice(idx, 0, qrs);
          } else {
  ...
  async duplicateQrSet() {
    const newName = await Popup.show.input(
      'Duplicate Quick Reply Set',
      'Enter a name for the new Quick Reply Set:',
      `${this.currentQrSet.name} (Copy)`
    );
    if (newName && newName.length > 0) {
      const existingSet = QuickReplySet.get(newName);
      if (existingSet) {
      ...
      newQrSet.init();
      const idx = QuickReplySet.list.findIndex(
        (it) => it.name.toLowerCase().localeCompare(newName.toLowerCase()) == 1
      );
      if (idx > -1) {
