...
export function getScriptsByType(scriptType, { allowedOnly } = DEFAULT_GET_REGEX_SCRIPTS_OPTIONS) {
  switch (scriptType) {
    ...
    case SCRIPT_TYPES.PRESET: {
      if (
        allowedOnly &&
        !extension_settings?.preset_allowed_regex?.[getCurrentPresetAPI()]?.includes(
  ...
  allRegex.forEach((script) => {
    if (
      ...
      if (isEdit && !script.runOnEdit) {
        console.debug(
          `getRegexedString: Skipping script ${script.scriptName} because it does not run on edit`
        );
        return;
      }
      ...
      if (typeof depth === 'number') {
        if (
          ...
          console.debug(
            `getRegexedString: Skipping script ${script.scriptName} because depth ${depth} is less than minDepth ${script.minDepth}`
          );
          return;
        }
        ...
        if (
          ...
          console.debug(
            `getRegexedString: Skipping script ${script.scriptName} because depth ${depth} is greater than maxDepth ${script.maxDepth}`
          );
          return;
        }
      }
