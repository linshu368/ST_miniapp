...
class RegexPresetManager {
  ...
  async checkUnsavedChanges() {
    ...
    const currentPreset = extension_settings.regex_presets.find(
      (p) => p.id === this.currentPresetId
    );
  ...
  setupEventListeners() {
    ...
    this.presetSelect.addEventListener('change', async (event) => {
      ...
      const fromSlashCommand =
        event instanceof CustomEvent && event?.detail?.fromSlashCommand === true;
        ...
        callback: (args, name) => {
          ...
          if (name) {
            ...
            const foundId = extension_settings.regex_presets.find(
              (p) =>
                equalsIgnoreCaseAndAccents(p.id, name) || equalsIgnoreCaseAndAccents(p.name, name)
            )?.id;

            if (foundId) {
              this.presetSelect.value = foundId;
              this.presetSelect.dispatchEvent(
                new CustomEvent('change', { detail: { fromSlashCommand: true } })
              );
              return foundId;
            }
  ...
  renderPresetList() {
    ...
    if (
      !Array.isArray(extension_settings.regex_presets) ||
      extension_settings.regex_presets.length === 0
    ) {
  ...
  async savePreset(presetId, isUpdate) {
    const existingPreset = isUpdate
      ? extension_settings.regex_presets.find((p) => p.id === presetId)
      : null;
  ...
  async deletePreset(presetId) {
    ...
    const confirm = await Popup.show.confirm(
      t`Are you sure you want to delete this regex preset?`,
      presetName
    );
    if (!confirm) {
      return;
...
function setMoveButtonsVisibility() {
  const hasGlobalScripts =
    $('#saved_regex_scripts .regex-script-label:has(.regex_bulk_checkbox:checked)').length > 0;
  const hasScopedScripts =
    $('#saved_scoped_scripts .regex-script-label:has(.regex_bulk_checkbox:checked)').length > 0;
  const hasPresetScripts =
    $('#saved_preset_scripts .regex-script-label:has(.regex_bulk_checkbox:checked)').length > 0;
...
async function saveRegexScript(regexScript, existingScriptIndex, scriptType, saveSettings = true) {
  ...
  if (regexScript.findRegex.length === 0) {
    toastr.warning(
      t`This regex script will not work, but was saved anyway: A find regex isn't present.`
    );
  }
...
async function loadRegexScripts() {
  ...
  getScriptsByType(SCRIPT_TYPES.GLOBAL).forEach((script, index) =>
    renderScript('#saved_regex_scripts', script, SCRIPT_TYPES.GLOBAL, index)
  );
  getScriptsByType(SCRIPT_TYPES.SCOPED).forEach((script, index) =>
    renderScript('#saved_scoped_scripts', script, SCRIPT_TYPES.SCOPED, index)
  );
  getScriptsByType(SCRIPT_TYPES.PRESET).forEach((script, index) =>
    renderScript('#saved_preset_scripts', script, SCRIPT_TYPES.PRESET, index)
  );

  $('#regex_scoped_toggle').prop('checked', isScopedScriptsAllowed(characters?.[this_chid]));
  $('#regex_preset_toggle').prop(
    'checked',
  ...
  while ((reMatch = backrefRegex.exec(pattern)) !== null) {
    ...
    if (backref === '$$') {
      ...
      container.appendChild(
        document.createTextNode(match.input.substring(match.index + match[0].length))
      );
    } else {
...
function populateDebuggerRuleList(container) {
  ...
  if (!allScripts || allScripts.length === 0) {
    rulesContainer.append(
      '<div class="regex-debugger-no-rules">' + t`No regex rules found.` + '</div>'
    );
    return;
  }
  ...
  const renderRule = (script) => {
    ...
    ruleElement
      .find('.edit_rule')
      .on('click', () => onRegexEditorOpenClick(script.id, script.type));
    ...
    ruleElement.on('click', function (event) {
      ...
      if (stepElement.length && container.length) {
        ...
        let scrollTo =
          containerScrollTop + targetTop - containerHeight / 2 + stepElement.height() / 2;
  ...
  if (globalScripts.length > 0) {
    rulesContainer.append(
      '<div class="list-header regex-debugger-list-header">' + t`Global Rules` + '</div>'
    );
  ...
  if (presetScripts.length > 0) {
    rulesContainer.append(
      '<div class="list-header regex-debugger-list-header">' + t`Preset Rules` + '</div>'
    );
  ...
  if (scopedScripts.length > 0) {
    rulesContainer.append(
      '<div class="list-header regex-debugger-list-header">' + t`Scoped Rules` + '</div>'
    );
...
async function onRegexDebuggerOpenClick() {
  ...
  debuggerHtml.find('#regex_debugger_expand_steps').on('click', function () {
    ...
    callGenericPopup(popupContainer, POPUP_TYPE.TEXT, t`Step-by-step Transformation`, {
      wide: true,
  ...
  debuggerHtml.find('#regex_debugger_expand_final').on('click', function () {
    ...
    callGenericPopup(popupContent, POPUP_TYPE.TEXT, t`Final Output`, {
      wide: true,
      large: true,
      allowVerticalScrolling: true,
    });
...
function updateInfoBlock(editorHtml) {
  ...
  try {
    ...
    const flagInfo = [];
    flagInfo.push(
      regex.flags.includes('g') ? t`Applies to all matches` : t`Applies to the first match`
    );
...
export async function init() {
  ...
  $('#bulk_regex_move_to_global').on('click', async () => {
    const confirm = await callGenericPopup(
      t`Are you sure you want to move the selected regex scripts to global?`,
      POPUP_TYPE.CONFIRM
    );
    if (!confirm) {
      return;
  ...
  $('#bulk_regex_move_to_scoped').on('click', async () => {
    ...
    const confirm = await callGenericPopup(
      t`Are you sure you want to move the selected regex scripts to scoped?`,
      POPUP_TYPE.CONFIRM
    );
    if (!confirm) {
      return;
  ...
  $('#bulk_regex_move_to_preset').on('click', async function () {
    const confirm = await callGenericPopup(
      t`Are you sure you want to move the selected regex scripts to preset?`,
      POPUP_TYPE.CONFIRM
    );
    if (!confirm) {
      return;
  ...
  $('#bulk_delete_regex').on('click', async function () {
    ...
    const confirm = await callGenericPopup(
      t`Are you sure you want to delete the selected regex scripts?`,
      POPUP_TYPE.CONFIRM
    );
    if (!confirm) {
      return;
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      helpString:
        'Runs a Regex extension script by name on the provided string. The script must be enabled.',
    })
  );
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
