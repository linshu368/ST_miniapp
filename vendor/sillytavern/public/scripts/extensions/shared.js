...
export async function getMultimodalCaption(base64Img, prompt) {
  ...
  const thumbnailNeeded = ['google', 'openrouter', 'mistral', 'groq', 'vertexai'].includes(
    extension_settings.caption.multimodal_api
  );
...
function throwIfInvalidModel(useReverseProxy) {
  ...
  if (
    altEndpointEnabled &&
    ['llamacpp', 'ooba', 'koboldcpp', 'vllm', 'ollama'].includes(multimodalApi) &&
    !altEndpointUrl
  ) {
  ...
  if (
    multimodalApi === 'ollama' &&
    multimodalModel === 'ollama_current' &&
    !textgenerationwebui_settings.ollama_model
  ) {
  ...
  if (
    multimodalApi === 'ollama' &&
    multimodalModel === 'ollama_custom' &&
    !extension_settings.caption.ollama_custom_model
  ) {
  ...
  if (
    multimodalApi === 'vllm' &&
    multimodalModel === 'vllm_current' &&
    !textgenerationwebui_settings.vllm_model
  ) {
  ...
  if (
    multimodalApi === 'custom' &&
    multimodalModel === 'custom_custom' &&
    !extension_settings.caption.custom_model
  ) {
...
export function isWebLlmSupported() {
  if (!('gpu' in navigator)) {
    ...
    if (!sessionStorage.getItem(warningKey)) {
      toastr.error(
        'Your browser does not support the WebGPU API. Please use a different browser.',
        'WebLLM',
        {
...
export class ConnectionManagerRequestService {
  ...
  static async sendRequest(
    ...
    prompt,
    maxTokens,
    custom = this.defaultSendRequestParams,
    overridePayload = {}
  ) {
      ...
      switch (selectedApiMap.selected) {
        case 'openai': {
          if (!selectedApiMap.source) {
            throw new Error(
              `API type ${selectedApiMap.selected} does not support chat completions`
            );
          }
        ...
        case 'textgenerationwebui': {
          if (!selectedApiMap.type) {
            throw new Error(
              `API type ${selectedApiMap.selected} does not support text completions`
            );
          }
  ...
  static constructPrompt(prompt, profileId, instructSettings = null) {
    ...
    switch (selectedApiMap.selected) {
      ...
      case 'textgenerationwebui': {
        ...
        return context.TextCompletionService.constructPrompt(
          prompt,
          instructName,
          instructSettings
        );
  ...
  static getProfileIcon(profileId) {
    if (
      SillyTavern.getContext().extensionSettings.disabledExtensions.includes('connection-manager')
    ) {
      return null;
    }

    const id =
      profileId ?? SillyTavern.getContext().extensionSettings.connectionManager.selectedProfile;
    if (!id) return null;
  ...
  static validateProfile(profile) {
    ...
    if (!Object.hasOwn(this.getAllowedTypes(), selectedApiMap.selected)) {
      throw new Error(
        `API type ${selectedApiMap.selected} is not supported. Supported types: ${Object.values(this.getAllowedTypes()).join(', ')}`
      );
    }
