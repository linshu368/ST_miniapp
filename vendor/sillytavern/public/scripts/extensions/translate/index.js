...
function showKeysButton() {
  ...
  $('#translate_key_button')
    .toggle(providerRequiresKey)
    .data('key', extension_settings.translate.provider);
...
function loadSettings() {
  ...
  $(`#translation_provider option[value="${extension_settings.translate.provider}"]`).attr(
    'selected',
    'true'
  );
  $(
  ...
  $(`#translation_auto_mode option[value="${extension_settings.translate.auto_mode}"]`).attr(
    'selected',
    'true'
  );
  $('#deepl_api_endpoint')
    .val(extension_settings.translate.deepl_endpoint)
    .toggle(extension_settings.translate.provider === 'deepl');
  showKeysButton();
}
...
async function translateIncomingMessageReasoning(messageId) {
  ...
  const textToTranslate = substituteParams(message.extra.reasoning, {
...
async function translateMessageEdit(messageId) {
  ...
  if (
    message.is_system ||
    (extension_settings.translate.auto_mode == autoModeOptions.NONE && message.extra?.display_text)
  ) {
...
export async function init() {
    ...
    (eventType) => {
      eventSource.on(eventType, (/** @type {string} */ key) => {
        if (key === extension_settings.translate.provider) {
          $('#translate_key_button').toggleClass(
            'success',
            !!secret_state[extension_settings.translate.provider]
          );
        }
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      name: 'translate',
      helpString:
        'Translate text to a target language. If target language is not provided, the value from the extension settings will be used.',
      namedArgumentList: [
        new SlashCommandNamedArgument(
          'target',
          'The target language code to translate to',
        ...
        SlashCommandNamedArgument.fromProps({
          name: 'provider',
          description:
            'The translation provider to use. If not provided, the value from the extension settings will be used.',
          typeList: [ARGUMENT_TYPE.STRING],
          isRequired: false,
          acceptsMultiple: false,
          enumProvider: () =>
            Array.from(
              document.getElementById('translation_provider').querySelectorAll('option')
            ).map(
              (option) =>
