...
class ChatterboxTtsProvider {
  ...
  async fetchTtsVoiceObjects() {
    try {
      ...
      const predefinedResponse = await fetch(
        `${this.settings.provider_endpoint}/get_predefined_voices`
      );
      ...
      console.log(
        `Loaded ${this.voices.length} voices (${predefinedVoices.length} predefined, ${referenceVoices.length} reference)`
      );
      return this.voices;
    } catch (error) {
  ...
  async getVoice(voiceName) {
    ...
    let match = this.voices.find(
      (voice) =>
        voice.name === voiceName || voice.voice_id === voiceName || voice.display_name === voiceName
    );
