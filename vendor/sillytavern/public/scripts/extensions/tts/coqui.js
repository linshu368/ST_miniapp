...
class CoquiTtsProvider {
  ...
  async loadSettings(settings) {
    ...
    let that = this;
    $('#coqui_model_origin').on('change', function () {
      that.onModelOriginChange();
    });
    $('#coqui_api_language').on('change', function () {
      that.onModelLanguageChange();
    });
    $('#coqui_api_model_name').on('change', function () {
      that.onModelNameChange();
    });

    $('#coqui_remove_voiceId_mapping').on('click', function () {
      that.onRemoveClick();
    });
    $('#coqui_add_voiceId_mapping').on('click', function () {
      that.onAddClick();
    });
  ...
  async onAddClick() {
    ...
    if (!voiceName) {
      toastr.error(
        'Voice name empty, please enter one.',
        DEBUG_PREFIX + ' voice mapping voice name',
    ...
    if (model_origin == 'none') {
      toastr.error(
        'Origin not selected, please select one.',
        DEBUG_PREFIX + ' voice mapping origin',
        { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true }
      );
      this.updateCustomVoices(); // Overide any manual modification
      return;
    ...
    if (model_origin == 'local') {
      ...
      if (model_name == 'none') {
        toastr.error(
          'Model not selected, please select one.',
          DEBUG_PREFIX + ' voice mapping model',
          { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true }
        );
        this.updateCustomVoices(); // Overide any manual modification
        return;
      ...
      this.settings.voiceMapDict[voiceName] = {
        model_type: 'local',
        model_id: 'local/' + model_id,
      };
      console.debug(
        DEBUG_PREFIX,
        'Registered new voice map: ',
        voiceName,
        ':',
        this.settings.voiceMapDict[voiceName]
      );
    ...
    if (model_language == 'none') {
      toastr.error(
        'Language not selected, please select one.',
        DEBUG_PREFIX + ' voice mapping language',
        { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true }
      );
      this.updateCustomVoices(); // Overide any manual modification
      return;
    ...
    if (model_origin == 'coqui-api-full') modelDict = coquiApiModelsFull;

    if (
      (model_setting_language == null) &
      ('languages' in modelDict[model_language][model_dataset][model_label])
    ) {
      toastr.error(
        'Model language not selected, please select one.',
        DEBUG_PREFIX + ' voice mapping model language',
        { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true }
      );
      return;
    }
    ...
    if (
      ...
      ('speakers' in modelDict[model_language][model_dataset][model_label])
    ) {
      toastr.error(
        'Model speaker not selected, please select one.',
        DEBUG_PREFIX + ' voice mapping model speaker',
        { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true }
      );
      return;
    }
    ...
    this.settings.voiceMapDict[voiceName] = {
      model_type: 'coqui-api',
      model_id: model_id,
      model_language: model_setting_language,
      model_speaker: model_setting_speaker,
    };
    ...
    if (model_setting_language != null) successMsg += '[' + model_setting_language + ']';
    if (model_setting_speaker != null) successMsg += '[' + model_setting_speaker + ']';
    toastr.info(successMsg, DEBUG_PREFIX + ' voice map updated', {
      timeOut: 10000,
  ...
  async onRemoveClick() {
    ...
    if (voiceName === 'none') {
      toastr.error(
        'Voice not selected, please select one.',
        DEBUG_PREFIX + ' voice mapping voiceId',
        { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true }
      );
      return;
    }
  ...
  async onModelNameChange() {
    ...
    if (model_state == 'installed') {
      ...
      if (model_state == 'corrupted') {
        ...
        $('#coqui_api_model_install_status').text(
          'Model found but incomplete try install again (maybe still downloading)'
        ); // (remove and download again)
      } else {
        toastr.info(
          'Click download button to install the model ' +
            $('#coqui_api_model_name').find(':selected').text(),
          DEBUG_PREFIX + ' model not installed',
          { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true }
        );
      ...
      $('#coqui_api_model_install_button')
        .off('click')
        .on('click', async function () {
          try {
            ...
            if (apiResultJSON.status == 'downloading') {
              toastr.error(
                'Check extras console for progress',
                DEBUG_PREFIX + ' already downloading',
                { timeOut: 10000, extendedTimeOut: 20000, preventDuplicates: true }
              );
              $('#coqui_api_model_install_status').text(
                'Already downloading a model, check extras console!'
              );
              $('#coqui_api_model_install_button').show();
            }
          } catch (error) {
            console.error(error);
            toastr.error(error, DEBUG_PREFIX + ' error with model download', {
              timeOut: 10000,
              extendedTimeOut: 20000,
              preventDuplicates: true,
            });
  ...
  static async installModel(model_id, action) {
    ...
    if (!apiResult.ok) {
      toastr.error(
        ...
        DEBUG_PREFIX + ' Install model ' + model_id + ' request failed'
      );
      throw new Error(`HTTP ${apiResult.status}: ${await apiResult.text()}`);
    }
  ...
  async fetchTtsVoiceObjects() {
    const voiceIds = Object.keys(this.settings.voiceMapDict).map((voice) => ({
      name: voice,
      voice_id: voice,
      preview_url: false,
    }));
