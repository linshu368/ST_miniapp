...
async function onNarrateText(args, text) {
  ...
  const name =
    (baseName === 'SillyTavern System' ? DEFAULT_VOICE_MARKER : baseName) || DEFAULT_VOICE_MARKER;

  const voiceMapEntry =
    voiceMap[name] === DEFAULT_VOICE_MARKER ? voiceMap[DEFAULT_VOICE_MARKER] : voiceMap[name];
...
function addAudioControl() {
  ...
  $('#ttsExtensionMenuItem')
    .attr('title', t`TTS play/pause`)
    .attr('data-i18n', '[title]TTS play/pause')
    .on('click', onAudioControlClicked);
  $('#ttsExtensionNarrateAll')
    .attr(
      'title',
      t`Narrate all messages in the current chat. Includes user messages, excludes hidden comments.`
    )
    .attr(
      'data-i18n',
      '[title]Narrate all messages in the current chat. Includes user messages, excludes hidden comments.'
    )
    .on('click', playFullConversation);
  updateUiAudioPlayState();
}
...
async function tts(text, voiceId, char, voiceMapKey = null) {
  ...
  await eventSource.emit(event_types.TTS_JOB_STARTED, {
    messageId,
    characterName: char,
    text,
    voiceId,
...
async function processTtsQueue() {
  ...
  if (currentTtsJob.segmentType && currentTtsJob.segmentText) {
    ...
    try {
      ...
      const voiceMapEntry =
        voiceMap[voiceMapKey] === DEFAULT_VOICE_MARKER
          ? voiceMap[DEFAULT_VOICE_MARKER]
          : voiceMap[voiceMapKey];
  ...
  let text = extension_settings.tts.narrate_translated_only
    ? currentTtsJob?.extra?.display_text || currentTtsJob.mes
    : currentTtsJob.mes;
...
function joinQuotedBlocks(text, opts = {}) {
  ...
  const cleaned = includeQuotes ? segments : segments.map((s) => s.slice(1, -1)); // all defined pairs are single-char quotes
...
function loadSettings() {
  ...
  $('#tts_periodic_auto_generation').prop(
    'checked',
    extension_settings.tts.periodic_auto_generation
  );
...
function onPeriodicAutoGenerationClick() {
  extension_settings.tts.periodic_auto_generation = !!$('#tts_periodic_auto_generation').prop(
    'checked'
  );
  saveSettingsDebounced();
}
...
function onNarrateTranslatedOnlyClick() {
  extension_settings.tts.narrate_translated_only = !!$('#tts_narrate_translated_only').prop(
    'checked'
  );
  saveSettingsDebounced();
}
...
async function onMessageDeleted() {
  ...
  const messageHash = getStringHash(
...
class VoiceMapEntry {
  ...
  addUI(voiceIds) {
    ...
    let defaultOption =
      this.name === DEFAULT_VOICE_MARKER
    ...
    SlashCommand.fromProps({
      ...
      namedArgumentList: [
        SlashCommandNamedArgument.fromProps({
          ...
          isRequired: false,
          enumProvider: () =>
            Object.keys(voiceMap).map(
              (voiceName) =>
                new SlashCommandEnumValue(voiceName, null, enumTypes.enum, enumIcons.voice)
            ),
        }),
      ],
