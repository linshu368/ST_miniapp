...
export class KokoroTtsProvider {
  constructor() {
    ...
    this.initTtsDebounced = debounceAsync(
      this.initializeWorker.bind(this),
  ...
  async loadSettings(settings) {
    ...
    $('#kokoro_speaking_rate')
      .val(this.settings.speakingRate)
      .on('input', this.onSettingsChange.bind(this));
  ...
  initializeWorker() {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Create a new worker
        this.worker = new Worker(new URL('./kokoro-worker.js', import.meta.url), {
          type: 'module',
        });
