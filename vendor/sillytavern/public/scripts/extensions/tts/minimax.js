...
class MiniMaxTtsProvider {
  ...
  async checkReady() {
    ...
    try {
      ...
      console.warn(
        'MiniMax TTS: Failed to fetch models/voices during ready check, will use all available:',
        error
      );
  ...
  async fetchTtsVoiceObjects() {
    try {
      if (!secret_state[SECRET_KEYS.MINIMAX] || !secret_state[SECRET_KEYS.MINIMAX_GROUP_ID]) {
        ...
        console.warn(
          'Using all available voices (default + custom). Please check your API credentials'
        );
  ...
  async fetchTtsGeneration(inputText, voiceId, language = null) {
    ...
    const requestBody = {
      ...
      audioSampleRate:
        Number(this.settings.audioSampleRate) || this.defaultSettings.audioSampleRate,
  ...
  async previewTtsVoice(voiceId) {
    ...
    try {
      ...
      this.audioElement.onerror = (e) => {
        ...
        toastr.error(
          'Audio playback failed. The audio format may not be supported by your browser.'
        );
      };
