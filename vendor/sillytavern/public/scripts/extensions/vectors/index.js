...
const settings = {
  ...
  summary_source: 'main',
  summary_prompt:
    'Ignore previous instructions. Summarize the most important parts of the message. Limit yourself to 250 words or less. Your response should include nothing but the summary.',
  summary_retries: 2,
...
const FATAL_CAUSES = new Set([
  ...
  'webllm_not_supported',
  'summary_endpoint_invalid',
]);
...
const remoteEmbeddingEndpoints = {
  ...
  electronhub: {
    ...
    textProperty: 'name',
    filter: (models) =>
      models.filter((m) => Array.isArray(m?.endpoints) && m.endpoints.includes('/v1/embeddings')),
  },
  openrouter: {
  ...
  try {
    ...
    while (!finished) {
      ...
      const processedPercent =
        initialPending > 0 ? Math.min(100, Math.round((processed / initialPending) * 100)) : 100;
    ...
    if (skippedHashes.size > 0) {
      toastr.warning(
        `${skippedHashes.size} message(s) skipped due to errors. Click Vectorize All again to retry.`,
...
async function summarizeOne(element, endpoint) {
  switch (endpoint) {
    ...
    default:
      throw new Error(`Unsupported summary endpoint: ${endpoint}`, {
        cause: 'summary_endpoint_invalid',
      });
  }
}
  ...
  for (const element of hashedMessages) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        console.warn(
          `Vectors: summary attempt ${attempt}/${maxAttempts} threw for hash ${element.hash}`,
          error
        );
      }
      console.warn(
        `Vectors: summary attempt ${attempt}/${maxAttempts} failed for hash ${element.hash}`
      );
    }
    if (!success) {
      if (skipOnFailure) {
        console.warn(
          `Vectors: summarization exhausted ${maxAttempts} attempt(s) for hash ${element.hash} — marking for skip`
        );
        element.summaryFailed = true;
        continue;
      }

      throw new Error(`Summarization failed after ${maxAttempts} attempt(s)`, {
...
async function synchronizeChat(batchSize = 5) {
  ...
  try {
    ...
    const hashedMessages = context.chat
      .filter((x) => settings.keep_hidden || !x.is_system)
      .map((x) => ({
    ...
    const deletedHashes = hashesInCollection.filter(
      (x) => !hashedMessages.some((y) => y.hash === x)
    );
    ...
    if (batch.length > 0) {
      ...
      console.log(
        `Vectors: Found ${newVectorItems.length} new items. Processing ${batch.length}...`
      );
      try {
...
async function processFiles(chat) {
  try {
    ...
    for (const message of chat) {
      ...
      const allFileText = String(message.mes || '')
        .substring(0, message.extra.fileLength)
        .trim();
...
async function ingestDataBankAttachments(source) {
  ...
  const dataBank = source
    ? getDataBankAttachmentsForSource(source, false)
    : getDataBankAttachments(false);
...
async function injectDataBankChunks(queryText, collectionIds) {
  try {
    ...
    for (const collectionId in queryResults) {
      console.debug(
        `Vectors: Processing Data Bank collection ${collectionId}`,
        queryResults[collectionId]
      );
      const metadata =
    ...
    setExtensionPrompt(
      ...
      insertedText,
      settings.file_position_db,
      settings.file_depth_db,
      settings.include_wi,
      settings.file_depth_role_db
    );
  } catch (error) {
...
async function retrieveFileChunks(queryText, collectionId) {
  ...
  console.debug(
    `Vectors: Retrieved ${queryResults.hashes.length} file chunks for collection ${collectionId}`,
    queryResults
  );
...
async function vectorizeFile(fileText, fileName, collectionId, chunkSize, overlapPercent) {
  ...
  try {
    ...
    toast = toastr.info(toastBody, `Ingesting file ${escapeHtml(fileName)}`, {
      closeButton: false,
      escapeHtml: false,
      timeOut: 0,
      extendedTimeOut: 0,
    ...
    console.debug(
      `Vectors: Split file ${fileName} into ${chunks.length} chunks with ${overlapPercent}% overlap`,
      chunks
    );
    ...
    for (let i = 0; i < items.length; i += batchSize) {
      toastBody.text(
        `${i}/${items.length} (${Math.round((i / items.length) * 100)}%) chunks processed`
      );
    ...
    toastr.clear(toast);
    console.log(
      `Vectors: Inserted ${chunks.length} vector items for file ${fileName} into ${collectionId}`
    );
    return true;
  } catch (error) {
    toastr.clear(toast);
...
async function rearrangeChat(chat, _contextSize, _abort, type) {
  try {
    ...
    toastr.error(
      'Generation interceptor aborted. Check browser console for more details.',
      'Vector Storage'
    );
...
function getPromptText(queriedMessages) {
  const queriedText = queriedMessages
    .map((x) => collapseNewlines(`${x.name}: ${x.mes}`).trim())
    .join('\n\n');
...
function overlapChunks(chunk, index, chunks, overlapSize) {
  ...
  const prevOverlap =
    trimToStartSentence(prevChunk?.substring(prevChunk.length - halfOverlap)) || '';
...
async function getQueryText(chat, initiator) {
  ...
  if (
    ...
    settings.summarize &&
    settings.summarize_sent
  ) {
    const minLength = Math.max(0, Number(settings.summary_threshold) || 0);
    const toSummarize =
      minLength > 0 ? hashedMessages.filter((x) => x.text.length >= minLength) : hashedMessages;
    if (toSummarize.length > 0) {
      await summarize(toSummarize, settings.summary_source, { skipOnFailure: true });
    }
  }
...
function throwIfSourceInvalid() {
  ...
  if (
    (settings.source === 'ollama' && !settings.ollama_model) ||
    (settings.source === 'vllm' && !settings.vllm_model)
  ) {
  ...
  try {
    ...
    switch (error.cause) {
      case 'webllm-not-available':
        toastr.warning(
          'WebLLM is not available. Please install the extension.',
          'WebLLM not installed'
        );
        break;
      case 'webllm-not-updated':
        toastr.warning(
          'The installed extension version does not support embeddings.',
...
async function createKoboldCppEmbeddings(items) {
  ...
  for (let i = 0; i < data.embeddings.length; i++) {
    if (!Array.isArray(data.embeddings[i]) || data.embeddings[i].length === 0) {
      throw new Error(
        'KoboldCpp returned an empty embedding. Reduce the chunk size and/or size threshold and try again.'
      );
    }
...
async function onViewStatsClick() {
  ...
  toastr.info(
    `Total hashes: <b>${totalHashes}</b><br>
...
async function onVectorizeAllFilesClick() {
  try {
    ...
    const chatAttachments = getContext()
      .chat.filter((x) => Array.isArray(x.extra?.files))
      .map((x) => x.extra.files)
      .flat();
    ...
    if (allSuccess) {
      ...
      toastr.warning(
        'Some files failed to vectorize. Check browser console for more details.',
        'Vector Storage'
      );
    }
  } catch (error) {
...
async function activateWorldInfo(chat) {
  ...
  for (const world in groupedEntries) {
    ...
    const hashesInCollection = await getSavedHashes(collectionId);
    const newEntries = groupedEntries[world].filter(
      (x) => !hashesInCollection.includes(getStringHash(x.content))
    );
    const deletedHashes = hashesInCollection.filter(
  ...
  const activatedHashes = Object.values(queryResults)
    .flatMap((x) => x.hashes)
    .filter(onlyUnique);
...
export async function init() {
  ...
  $('#vectors_keep_hidden')
    .prop('checked', settings.keep_hidden)
    .on('input', () => {
  ...
  $('#vectors_chutes_model')
    .val(settings.chutes_model)
    .on('change', () => {
  ...
  $('#vectors_size_threshold')
    .val(settings.size_threshold)
    .on('input', () => {
  ...
  $('#vectors_summarize')
    .prop('checked', settings.summarize)
    .on('input', () => {
  ...
  $('#vectors_summarize_user')
    .prop('checked', settings.summarize_sent)
    .on('input', () => {
  ...
  $('#vectors_summary_source')
    .val(settings.summary_source)
    .on('change', () => {
  ...
  $('#vectors_summary_prompt')
    .val(settings.summary_prompt)
    .on('input', () => {
  ...
  $('#vectors_summary_retries')
    .val(settings.summary_retries)
    .on('input', () => {
  ...
  $('#vectors_summary_threshold')
    .val(settings.summary_threshold)
    .on('input', () => {
  ...
  $('#vectors_size_threshold_db')
    .val(settings.size_threshold_db)
    .on('input', () => {
  ...
  $('#vectors_chunk_size_db')
    .val(settings.chunk_size_db)
    .on('input', () => {
  ...
  $('#vectors_chunk_count_db')
    .val(settings.chunk_count_db)
    .on('input', () => {
  ...
  $('#vectors_overlap_percent_db')
    .val(settings.overlap_percent_db)
    .on('input', () => {
  ...
  $('#vectors_file_template_db')
    .val(settings.file_template_db)
    .on('input', () => {
  ...
  $(`input[name="vectors_file_position_db"][value="${settings.file_position_db}"]`).prop(
    'checked',
    true
  );
  ...
  $('#vectors_file_depth_db')
    .val(settings.file_depth_db)
    .on('input', () => {
  ...
  $('#vectors_file_depth_role_db')
    .val(settings.file_depth_role_db)
    .on('input', () => {
  ...
  $('#vectors_translate_files')
    .prop('checked', settings.translate_files)
    .on('input', () => {
  ...
  $('#vectors_enabled_for_all')
    .prop('checked', settings.enabled_for_all)
    .on('input', () => {
  ...
  $('#vectors_only_custom_boundary')
    .prop('checked', settings.only_custom_boundary)
    .on('input', () => {
  ...
  [event_types.SECRET_WRITTEN, event_types.SECRET_DELETED, event_types.SECRET_ROTATED].forEach(
    (event) => {
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      helpString:
        'Search the Data Bank for a specific query using vector similarity. Returns a list of file URLs with the most relevant content.',
      namedArgumentList: [
        new SlashCommandNamedArgument(
          'threshold',
          'Threshold for the similarity score in the [0, 1] range. Uses the global config value if not set.',
        ...
        new SlashCommandNamedArgument(
          'source',
          'Optional filter for the attachments by source.',
        ...
        SlashCommandNamedArgument.fromProps({
          ...
          enumList: [
            new SlashCommandEnumValue(
              'chunks',
              'Return the actual content chunks',
              enumTypes.enum,
              '{}'
            ),
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      helpString:
        'Set the vector score threshold or return the current threshold if no argument is provided.',
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      name: 'vector-query',
      helpString:
        'Set the vector query messages or returns the current query messages count if no argument is provided',
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      helpString:
        'Set the vector world info max entries or returns the current max entries if no argument is provided',
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      helpString:
        'Set whether chat vectorization is enabled or return the current boolean if no argument is provided',
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      helpString:
        'Set whether file vectorization is enabled or return the current boolean if no argument is provided',
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      helpString:
        'Set whether world info vectorization is enabled or return the current boolean if no argument is provided',
  ...
  registerDebugFunction(
    'purge-everything',
    'Purge all vector indices',
    'Obliterate all stored vectors for all sources. No mercy.',
    async () => {
