...
export function isFilterState(a, b) {
  ...
  const aKey =
    typeof a == 'string' && states.includes(a) ? a : states.find((key) => FILTER_STATES[key] === a);
  const bKey =
    typeof b == 'string' && states.includes(b) ? b : states.find((key) => FILTER_STATES[key] === b);
...
export class FilterHelper {
  ...
  wiSearchFilter(data) {
    ...
    this.cacheScores(
      FILTER_TYPES.WORLD_INFO_SEARCH,
      new Map(fuzzySearchResults.map((i) => [i.item?.uid, i.score]))
    );
  ...
  personaSearchFilter(data) {
    ...
    this.cacheScores(
      FILTER_TYPES.PERSONA_SEARCH,
      new Map(fuzzySearchResults.map((i) => [i.item.key, i.score]))
    );
  ...
  filterDataByState(data, state, filterFunc, { includeFolders = false } = {}) {
    if (isFilterState(state, FILTER_STATES.SELECTED)) {
      return data.filter(
        (entity) => filterFunc(entity) || (includeFolders && entity.type == 'tag')
      );
    }
  ...
  searchFilter(data) {
    ...
    if (power_user.fuzzy_search) {
      ...
      this.cacheScores(
        FILTER_TYPES.SEARCH,
        new Map(fuzzySearchTagsResult.map((i) => [`tag.${i.item.id}`, i.score]))
      );
    }
  ...
  applyFilters(
    data,
    { clearScoreCache = true, tempOverrides = {}, clearFuzzySearchCaches = true } = {}
  ) {
