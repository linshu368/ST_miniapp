...
export function loadNovelPreset(preset) {
  if (preset.genamt === undefined) {
    const needsUnlock =
      preset.max_context > MAX_CONTEXT_DEFAULT || preset.max_length > MAX_RESPONSE_DEFAULT;
...
function loadNovelSettingsUi(ui_settings) {
  ...
  $('#rep_pen_slope_counter_novel').val(
  ...
  $('#rep_pen_presence_counter_novel').val(
  ...
  $('#math1_quad_entropy_scale_counter_novel').val(
    Number(ui_settings.math1_quad_entropy_scale).toFixed(2)
  );
  $(
    `#settings_preset_novel option[value=${novelai_setting_names[nai_settings.preset_settings_novel]}]`
  ).prop('selected', true);
...
export function initNovelAISettings() {
  ...
  $('#model_novel_select').on('change', function () {
    ...
    $(`#settings_preset_novel option[value=${novelai_setting_names[default_preset]}]`).attr(
      'selected',
      'true'
    );
  ...
  $('#novelai_logit_bias_new_entry').on('click', () =>
