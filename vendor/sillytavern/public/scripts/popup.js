...
export class Popup {
  ...
  constructor(
    ...
    this.dlg.addEventListener('focusin', (evt) => {
      if (evt.target instanceof HTMLElement && evt.target != this.dlg) this.lastFocus = evt.target;
    });
    ...
    this.dlg.querySelectorAll('[data-result]').forEach((resultControl) => {
      ...
      if (result !== null && isNaN(result))
        throw new Error(
          'Invalid result control. Result must be a number. ' + resultControl.dataset.result
        );
    ...
    const keyListener = async (evt) => {
      switch (evt.key) {
        case 'Enter': {
          ...
          if (
            (textarea instanceof HTMLTextAreaElement || input instanceof HTMLInputElement) &&
            !evt.ctrlKey &&
