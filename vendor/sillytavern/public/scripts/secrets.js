...
export function resolveSecretKey() {
  ...
  if (mainApi === 'textgenerationwebui') {
    const [key] = Object.entries(textgen_types).find(
      ([, value]) => value === textCompletionType
    ) ?? [null];
  ...
  if (mainApi === 'openai') {
    ...
    const [key] = Object.entries(chat_completion_sources).find(
      ([, value]) => value === chatCompletionSource
    ) ?? [null];
    if (key && SECRET_KEYS[key]) {
      return SECRET_KEYS[key];
    }
...
function registerSecretSlashCommands() {
  const secretKeyEnumProvider = () =>
    Object.values(SECRET_KEYS).map(
      (key) =>
        new SlashCommandEnumValue(key, FRIENDLY_NAMES[key] || key, enumTypes.name, enumIcons.key)
    );
  const secretIdEnumProvider = (
    ...
    const key =
      executor?.namedArgumentList?.find((x) => x.name === 'key')?.value?.toString() ||
      resolveSecretKey();
    if (
      !key ||
      !secret_state[key] ||
      !Array.isArray(secret_state[key]) ||
      secret_state[key].length === 0
    ) {
      return [];
    }
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      callback: async (args, value) => {
        ...
        const savedSecret =
          secrets.find((s) => s.id === id) ??
          secrets.find((s) => s.label === id) ??
          secrets.find((s) => s.active);
        if (!savedSecret) {
          if (!quiet) {
            toastr.error(t`No secret found with ID: ${id} for the key: ${key}`);
          }
  ...
  const createNanoGptCreditsPopup = (credits) => {
    ...
    const addUsage = (label, usage, limit) => {
      if (usage) {
        rows.push([
          label,
          t`${formatNanoGptNumber(usage.used)} / ${formatNanoGptNumber(limit)} (${formatNanoGptNumber(usage.remaining)} left)`,
        ]);
      }
    };
    ...
    if (credits.subscription?.active) {
      ...
      const subEndDate = sub.period?.currentPeriodEnd
        ? moment(sub.period.currentPeriodEnd).format('LL')
        : t`Unknown`;
