...
export const commonEnumProviders = {
      ...
      return [
        ...(isAll || types.includes('scope')
          ? scope.allVariableNames.map(
              (name) =>
                new SlashCommandEnumValue(name, null, enumTypes.variable, enumIcons.scopeVariable)
            )
          : []),
        ...(isAll || types.includes('local')
          ? Object.keys(chat_metadata.variables ?? []).map(
              (name) =>
                new SlashCommandEnumValue(name, null, enumTypes.name, enumIcons.localVariable)
            )
          : []),
        ...(isAll || types.includes('global')
          ? Object.keys(extension_settings.variables.global ?? []).map(
              (name) =>
                new SlashCommandEnumValue(name, null, enumTypes.macro, enumIcons.globalVariable)
            )
          : []),
    ...
    () => {
      return [
        ...(['all', 'character'].includes(mode)
          ? characters.map(
              (char) =>
                new SlashCommandEnumValue(char.name, null, enumTypes.name, enumIcons.character)
            )
          : []),
        ...(['all', 'group'].includes(mode)
          ? groups.map(
              (group) => new SlashCommandEnumValue(group.name, null, enumTypes.qr, enumIcons.group)
            )
          : []),
  ...
  worlds: () =>
    world_names.map(
      (worldName) => new SlashCommandEnumValue(worldName, null, enumTypes.name, enumIcons.world)
    ),
