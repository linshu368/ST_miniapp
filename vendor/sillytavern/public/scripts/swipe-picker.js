    ...
    const swipeBlocks = await Promise.all(
      message.swipes.map(async (swipe, index) => {
        ...
        deleteButton
          ...
          .each(function () {
            if (canDeleteSwipe) {
              ...
              $(this).removeAttr('title').removeAttr('data-i18n');
            }
          })
          .off('click')
          .on('click', async (event) => {
        ...
        template
          .find('.chat_file_size')
          .text(
            swipeDetails.length ? `(${swipeDetails[0]}${swipeDetails.length > 1 ? ',' : ')'}` : ''
          );
        template
          .find('.chat_messages_num')
          .text(swipeDetails.length > 1 ? `${swipeDetails.slice(1).join(', ')})` : '');
