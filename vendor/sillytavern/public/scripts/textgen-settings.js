...
function toggleBannedStringsKillSwitch(isEnabled, title) {
  ...
  $('#send_banned_tokens_label')
    .find('.menu_button')
    .toggleClass('toggleEnabled', isEnabled)
    .prop('title', title);
...
export async function loadTextGenSettings(data, loadedSettings) {
  ...
  if (loadedSettings.api_server_textgenerationwebui) {
    for (const type of Object.keys(SERVER_INPUTS)) {
      textgenerationwebui_settings.server_urls[type] =
        loadedSettings.api_server_textgenerationwebui;
    }
  ...
  $('#openrouter_providers_text')
    .val(textgenerationwebui_settings.openrouter_providers)
    .trigger('change');
  $('#openrouter_quantizations_text')
    .val(textgenerationwebui_settings.openrouter_quantizations)
    .trigger('change');
...
async function getStatusTextgen() {
  ...
  if (
    [textgen_types.GENERIC, textgen_types.OOBA].includes(textgenerationwebui_settings.type) &&
...
export function initTextGenSettings() {
      ...
      for (const [id, value] of Object.entries(inputs)) {
        ...
        if (inputElement.prop('type') === 'checkbox') {
          ...
          if (power_user.enableZenSliders) {
            ...
            zenSlider.slider('option', 'slide').call(zenSlider, null, {
              handle: $('.ui-slider-handle', zenSlider),
              value: value,
            });
          }
        }
  ...
  $('#textgen_logit_bias_new_entry').on('click', () =>
    createNewLogitBiasEntry(textgenerationwebui_settings.logit_bias, BIAS_KEY)
  );
...
function showSamplerControls(apiType = null) {
  ...
  const prioritizeManualSamplerSelect = isSamplerManualPriorityEnabled(
    apiType ?? textgenerationwebui_settings.type
  );
...
function setSettingByName(setting, value, trigger) {
  ...
  if ('json_schema' === setting) {
    ...
    $('#tabby_json_schema').val(
      value ? JSON.stringify(textgenerationwebui_settings.json_schema, null, 2) : ''
    );
    return;
  }
  ...
  const isText =
    $(`#${setting}_textgenerationwebui`).attr('type') == 'text' ||
    $(`#${setting}_textgenerationwebui`).is('textarea');
  if (isCheckbox) {
    ...
    while (true) {
      ...
      if (data?.choices?.[0]?.index > 0) {
        ...
        logprobs = parseTextgenLogprobs(
          newText,
          data.choices?.[0]?.logprobs || data?.completion_probabilities
        );
...
export function parseTabbyLogprobs(data) {
  ...
  const topLogprobs = data?.choices?.[0]?.logprobs?.top_logprobs?.map((x) => ({
    top_logprobs: [x],
  }));
...
export function isJsonSchemaSupported() {
  return (
    [TABBY, LLAMACPP].includes(textgenerationwebui_settings.type) &&
...
export function createTextGenGenerationData(
  ...
  const jsonSchema = isObject(settings.json_schema)
    ? settings.json_schema_allow_empty
      ...
      : Object.keys(settings.json_schema).length > 0
        ? settings.json_schema
        : undefined
    : undefined;
  ...
  const aphroditeParams = {
    ...
    sampler_priority:
      ...
      !arraysEqual(settings.samplers_priorities, APHRODITE_DEFAULT_ORDER)
  ...
  if (settings.type === HUGGINGFACE) {
    ...
    nonAphroditeParams.seed =
      settings.seed >= 0 ? settings.seed : Math.floor(Math.random() * Math.pow(2, 32));
  }
  ...
  if (settings.type === LLAMACPP || settings.type === OLLAMA) {
    ...
    const logitBiasArray =
      params.logit_bias &&
      typeof params.logit_bias === 'object' &&
      Object.keys(params.logit_bias).length > 0
