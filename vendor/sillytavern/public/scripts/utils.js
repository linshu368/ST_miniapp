import { moment, DOMPurify, Readability, isProbablyReaderable, lodash } from '../lib.js';
...
export async function fetchFaFile(name) {
  ...
  return [...sheet.cssRules]
    ...
    .map((rule) =>
      rule['selectorText'].split(/,\s*/).map((selector) => selector.split('::').shift().slice(1))
    );
}

export async function fetchFa() {
  return [...new Set((await Promise.all([fetchFaFile('fontawesome.min.css')])).flat())];
}
/**
...
export function findChar({
  ...
  if (preferCurrentChar) {
    ...
    if (preferredCharSearch.length > 1) {
      if (!quiet) toastr.warning(t`Multiple characters found for given conditions.`);
        ...
        console.warn(t`Multiple characters found for given conditions. Returning the first match.`);
    }
  ...
  if (allowAvatar && name) {
    const characterByAvatar = filteredCharacters.find(
      (char) => char.avatar === name || (!name.endsWith('.png') && char.avatar === `${name}.png`)
    );
...
export function setupScrollToTop({
  ...
  const updateButtonVisibilityThrottled = lodash.throttle(
