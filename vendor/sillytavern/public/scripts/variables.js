...
async function listVariablesCallback(args) {
  ...
  const globalVariables = includeGlobalVariables
    ? Object.entries(extension_settings.variables.global).map(
  ...
  const jsonVariables = [
    ...
    ...Object.entries(extension_settings.variables.global).map((x) => ({
      key: x[0],
      value: x[1],
      scope: 'global',
    })),
...
export function existsGlobalVariable(name) {
  return (
    extension_settings.variables.global && extension_settings.variables.global[name] !== undefined
  );
}
...
function sortArrayObjectCallback(args, value) {
  ...
  if (Array.isArray(parsedValue)) {
    ...
    if (isFalseBoolean(keysort)) {
      parsedValue = Object.keys(parsedValue).sort(function (a, b) {
...
export function registerVariableCommands() {
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      callback: async (_, value) =>
        deleteLocalVariable(
          value instanceof SlashCommandClosure ? (await value.execute())?.pipe : String(value)
        ),
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      callback: (args, value) =>
        String(
          randValuesCallback(Number(args.from ?? 0), Number(args.to ?? (value ? value : 1)), args)
        ),
