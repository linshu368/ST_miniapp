...
export class DataMaidService {
  ...
  async sanitizeReport(report) {
    const sanitizedReport = {
      ...
      avatarThumbnails: await Promise.all(
        report.avatarThumbnails.map((i) => this.#sanitizeRecord(i, false))
      ),
      backgroundThumbnails: await Promise.all(
        report.backgroundThumbnails.map((i) => this.#sanitizeRecord(i, false))
      ),
      personaThumbnails: await Promise.all(
        report.personaThumbnails.map((i) => this.#sanitizeRecord(i, false))
      ),
      chatBackups: await Promise.all(report.chatBackups.map((i) => this.#sanitizeRecord(i, false))),
      settingsBackups: await Promise.all(
        report.settingsBackups.map((i) => this.#sanitizeRecord(i, false))
      ),
    };
  ...
  async #collectFiles() {
    ...
    try {
      ...
      const metadata = await this.#parseAllMetadata(
        (x) => Array.isArray(x?.attachments) && x.attachments.length > 0
      );
      for (const meta of metadata) {
  ...
  async #collectPersonaThumbnails() {
    ...
    try {
      ...
      const personaThumbnails = await fs.promises.readdir(this.directories.thumbnailsPersona, {
        withFileTypes: true,
      });
