...
comfy.post('/generate', async (request, response) => {
  try {
    ...
    if (item.status.status_str === 'error') {
      ...
      const errorMessages =
        item.status?.messages
          ...
          .map((it) => it[1])
          .map(
            (it) => `${it.node_type} [${it.node_id}] ${it.exception_type}: ${it.exception_message}`
          )
          .join('\n') || '';
    ...
    const imgInfo =
      outputs.map((it) => it.images).flat()[0] ?? outputs.map((it) => it.gifs).flat()[0];
    if (!imgInfo) {
...
electronhub.post('/models', async (request, response) => {
  try {
    ...
    const models = data.data
      .filter(
        (x) => x && Array.isArray(x.endpoints) && x.endpoints.includes('/v1/images/generations')
      )
...
chutes.post('/models', async (request, response) => {
  try {
    ...
    const models = chutesData.items
      .map((x) => ({ value: x.name, text: x.name }))
...
aimlapi.post('/models', async (request, response) => {
  try {
    ...
    const models = (data.data || [])
      .filter(
        (model) =>
          model.type === 'image' && model.id !== 'triposr' && model.id !== 'flux/dev/image-to-image'
      )
      .map((model) => ({
        value: model.id,
