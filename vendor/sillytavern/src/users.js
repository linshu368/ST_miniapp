...
function logSecurityAlert(message) {
  ...
  if (getConfigValue('securityOverride', false, 'boolean')) {
    console.warn(
      color.red("Security has been overridden. If it's not a trusted network, change the settings.")
    );
    return;
  }
  process.exit(1);
  ...
  for (const directory of directories) {
    try {
      ...
      migratedPrompts = migratedPrompts.filter(
        (x) => !defaultPrompts.some((y) => y.content === x.content)
      );
...
async function headerUserLogin(request, header = 'Remote-User') {
  ...
  if (!isTrusted) {
    console.warn(
      color.yellow(`Received ${header} header from untrusted IP ${ip}. Ignoring for auto-login.`)
    );
    return false;
  }
