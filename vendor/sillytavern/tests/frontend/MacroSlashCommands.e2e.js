      ...
      const output = await page.evaluate(async () => {
        ...
        return (
          await executeSlashCommandsWithOptions(
            '/parser-flag REPLACE_GETVAR || /setvar key=x \\{\\{lastMessageId}} || /pass {{getvar::x}}'
          )
        ).pipe;
      });
  ...
  test.describe('{{var}} Macro', () => {
    ...
    test('should not fail on out-of-bounds variable index', async ({ page }) => {
      const output = await page.evaluate(async () => {
        ...
        return (
          await executeSlashCommandsWithOptions(
            '/let key=test {"key":"value"} || /pass {{var::test::error}}'
          )
        ).pipe;
      });
  ...
  test.describe('{{arg}} Macro', () => {
    test('should support {{arg}} macro with plain value', async ({ page }) => {
      const output = await page.evaluate(async () => {
        ...
        return (
          await executeSlashCommandsWithOptions('/qr-arg hello world || /pass {{arg::hello}}')
        ).pipe;
      });
    ...
    test('should support {{arg}} macro with closure value', async ({ page }) => {
      const output = await page.evaluate(async () => {
        ...
        return (
          await executeSlashCommandsWithOptions('/qr-arg x {: /echo test :} || /echo {{arg::x}}')
        ).pipe;
      });
    ...
    test('should support mixed type {{arg}} macro values', async ({ page }) => {
      const output = await page.evaluate(async () => {
        ...
        return (
          await executeSlashCommandsWithOptions(
            '/qr-arg a simple || /qr-arg b {: /echo closure :} || /echo {{arg::a}} and {{arg::b}}'
          )
        ).pipe;
      });
    ...
    test('should support wildcard {{arg}} macro', async ({ page }) => {
      const output = await page.evaluate(async () => {
        ...
        return (await executeSlashCommandsWithOptions('/qr-arg * wildcard || /pass {{arg::any}}'))
          .pipe;
      });
  ...
  test.describe('Custom Scope Macros', () => {
    ...
    test('should support custom SlashCommandScope macros', async ({ page }) => {
      const output = await page.evaluate(async () => {
        ...
        return (
          await executeSlashCommandsWithOptions('/pass {{uno::dos::tres}}', { scope: customScope })
        ).pipe;
      });
