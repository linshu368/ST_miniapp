...
test.describe('MacroStoryString', () => {
  ...
  test.beforeAll(() => {
    const contextPresetsPath = path.join(
      serverDirectory,
      'default',
      'content',
      'presets',
    ...
    const files = fs
      .readdirSync(contextPresetsPath)
    ...
    const output = await page.evaluate(
      async ([defaultContextPresets]) => {
        ...
        function getMacroStoryString(templateString) {
          let output = substituteParams(templateString, {
            name1Override: context.user,
            name2Override: context.char,
            replaceCharacterCard: true,
            dynamicMacros: context,
          });
        ...
        for (const template of defaultContextPresets) {
          const classicStoryString = renderStoryString(context, {
            customStoryString: template.story_string,
